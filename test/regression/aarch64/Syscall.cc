...
TEST_P(Syscall, getcwd) {
  ...
  auto data = memory_->getUntimedData(
      paddr, std::filesystem::current_path().generic_string().size());
  for (int i = 0; i < std::filesystem::current_path().generic_string().size();
       i++) {
    EXPECT_EQ(data[i], std::filesystem::current_path().c_str()[i])
        << "at index i=" << i << '\n';
  }
}
...
TEST_P(Syscall, filenotfound) {
  ...
  EXPECT_EQ(getGeneralRegister<uint64_t>(0), -2);
}
