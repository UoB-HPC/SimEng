...
TEST_P(MicroOp, storePairD) {
  ...
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 1024), -5.0);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 1016), -3.5);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 1008), 3.5);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 1000), 5.0);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 992), -1.5);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 984), -0.5);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 976), 0.5);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 968), 1.5);
}
...
TEST_P(MicroOp, storePairQ) {
  ...
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1024),
            0xABBACAFEABBACAFE);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1016),
            0x1234567898765432);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1008),
            0xABCDEFABCDEFABCD);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1000),
            0xCAFEABBACAFEABBA);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 992),
            0x9876543212345678);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 984),
            0xFEDCBAFEDCBAFEDC);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 976),
            0xABBACAFEABBACAFE);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 968),
            0x1234567898765432);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 960),
            0x9876543212345678);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 952),
            0xFEDCBAFEDCBAFEDC);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 944),
            0xABBACAFEABBACAFE);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 936),
            0x1234567898765432);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 928),
            0xABBACAFEABBACAFE);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 920),
            0x1234567898765432);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 912),
            0xABCDEFABCDEFABCD);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 904),
            0xCAFEABBACAFEABBA);
}
...
TEST_P(MicroOp, storePairS) {
  ...
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1024), -5.0f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1020), -3.5f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1016), 3.5f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1012), 5.0f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1008), -1.5f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1004), -0.5f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1000), 0.5f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 996), 1.5f);
}
...
TEST_P(MicroOp, storePairW) {
  ...
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1024), 12);
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1020), 24);
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1016), 84);
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1012), 96);
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1008), 36);
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1004), 48);
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1000), 60);
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 996), 72);
}
...
TEST_P(MicroOp, storePairX) {
  ...
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1024), 12);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1016), 24);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1008), 84);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1000), 96);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 992), 36);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 984), 48);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 976), 60);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 968), 72);
}
...
TEST_P(MicroOp, storeB) {
  ...
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialProcessStackPointer() - 1024), 0xAB);
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialProcessStackPointer() - 1023), 0xFE);
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialProcessStackPointer() - 1022), 0xBA);
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialProcessStackPointer() - 1021), 0xCA);
}
...
TEST_P(MicroOp, storeD) {
  ...
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 1024), -3.0);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 1016), 3.0);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 1008), -1.5);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialProcessStackPointer() - 1000), 1.5);
}
...
TEST_P(MicroOp, storeH) {
  ...
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialProcessStackPointer() - 1024),
            0xABBA);
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialProcessStackPointer() - 1022),
            0x5678);
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialProcessStackPointer() - 1020),
            0xCAFE);
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialProcessStackPointer() - 1018),
            0x1234);
}
...
TEST_P(MicroOp, storeS) {
  ...
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1024), -3.0f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1020), 3.0f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1016), -1.5f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialProcessStackPointer() - 1012), 1.5f);
}
...
TEST_P(MicroOp, storeW) {
  ...
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1024), 12);
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1020), 48);
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1016), 24);
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getInitialProcessStackPointer() - 1012), 36);
}
...
TEST_P(MicroOp, storeX) {
  ...
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1024), 12);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1016), 48);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1008), 24);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialProcessStackPointer() - 1000), 36);
}
