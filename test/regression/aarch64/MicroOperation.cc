...
TEST_P(MicroOp, stur) {
  ...
  EXPECT_EQ(getMemoryValue<uint32_t>(process_->getStackPointer() - 1024), 1);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getStackPointer() - 1020), 2);
  EXPECT_EQ(getMemoryValue<double>(process_->getStackPointer() - 1012), 3);
  EXPECT_EQ(getMemoryValue<double>(process_->getStackPointer() - 1004), 4);
  EXPECT_EQ(getMemoryValue<double>(process_->getStackPointer() - 996), 4);
  EXPECT_EQ(getMemoryValue<float>(process_->getStackPointer() - 988), 5);
}
