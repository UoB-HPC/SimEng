...
TEST_P(MicroOp, storeB) {
  ...
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialStackPointer() - 512),
            1);
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialStackPointer() - 511),
            4);
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialStackPointer() - 510),
            2);
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialStackPointer() - 509),
            3);
}
...
TEST_P(MicroOp, storeExtendedB) {
  ...
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialStackPointer() - 1021),
            0xCA);
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialStackPointer() - 1020),
            0xFE);
  ...
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialStackPointer() - 509),
            3);
  EXPECT_EQ(getMemoryValue<uint8_t>(process_->getInitialStackPointer() - 508),
            4);
}
...
TEST_P(MicroOp, storeExtendedD) {
  ...
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialStackPointer() - 1008),
            -1.5);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialStackPointer() - 960),
            1.5);
  EXPECT_EQ(getMemoryValue<double>(process_->getInitialStackPointer() - 896),
            3.0);
}
...
TEST_P(MicroOp, storeH) {
  ...
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialStackPointer() - 512),
            1);
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialStackPointer() - 510),
            4);
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialStackPointer() - 508),
            2);
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialStackPointer() - 506),
            3);
}
...
TEST_P(MicroOp, storeExtendedH) {
  ...
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialStackPointer() - 1020),
            0xCAFE);
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialStackPointer() - 1012),
            0x1234);
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialStackPointer() - 1008),
            0x5678);
  ...
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialStackPointer() - 508),
            2);
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialStackPointer() - 500),
            3);
  EXPECT_EQ(getMemoryValue<uint16_t>(process_->getInitialStackPointer() - 496),
            4);
}
...
TEST_P(MicroOp, storeExtendedQ) {
  ...
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialStackPointer() - 984),
            0xCAFEABBACAFEABBA);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialStackPointer() - 768),
            0x9876543212345678);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialStackPointer() - 760),
            0xFEDCBAFEDCBAFEDC);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialStackPointer() - 512),
            0xABBACAFEABBACAFE);
  EXPECT_EQ(getMemoryValue<uint64_t>(process_->getInitialStackPointer() - 504),
            0x1234567898765432);
}
...
TEST_P(MicroOp, storeExtendedS) {
  ...
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialStackPointer() - 1008),
            1.5f);
  EXPECT_EQ(getMemoryValue<float>(process_->getInitialStackPointer() - 992),
            3.0f);
}
