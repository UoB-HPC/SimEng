...
//   uint64_t mmapStart =
//   simOS.getProcess(procTID)->getMemRegion().getMmapStart();
...
//   uint64_t paddrWOffset = simOS.getProcess(procTID)->translate(mmapStart +
//   20); ASSERT_EQ(paddrWOffset, paddr + 20);
// }
...
//   int fd = process->fdArray_->allocateFDEntry(0, fpath.c_str(), O_RDWR,
//   0666); ASSERT_NE(fd, -1);

//   // Inject fake syscall into queue
//   simOS.getSyscallHandler()->receiveSyscall({});

//   uint64_t retVal = simOS.getSyscallHandler()->mmap(mmapStart, 21, 0, 0, fd,
//   0); ASSERT_NE(retVal, 0);
//   ASSERT_EQ(simOS.getProcess(procTID)->getMemRegion().getVMASize(), 1);
