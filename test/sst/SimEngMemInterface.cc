...
void SimEngMemInterface::splitAggregatedRequest(AggregateWriteRequest* aggrReq,
  ...
  */
  ...
  for (int x = 0; x < numCacheLinesNeeded; x++) {
    ...
    // Marked as non-cacheable if untimed
    if (aggrReq->pkt_->isUntimed() && aggrReq->pkt_->isFromSystem()) {
      flags = (StandardMem::Request::flags_t)
          StandardMem::Request::Flag::F_NONCACHEABLE;
    }
    ...
    if (aggrReq->pkt_->isAtomic()) {
      ...
      if (print)
        std::cout << "\t\tAtomic write access at " << std::hex << vAddrStart
                  << std::dec << " from instruction "
                  << aggrReq->pkt_->insnSeqId_ << " with id "
                  << writeReq->getID() << std::endl;
    } else {
    ...
    // Couple with cache line flush for effected line(s) if untimed
    if (aggrReq->pkt_->isUntimed() && aggrReq->pkt_->isFromSystem()) {
      StandardMem::FlushAddr* flushReq = new StandardMem::FlushAddr(
          pAddrStart, currReqSize, true, 2, 0, vAddrStart,
          aggrReq->pkt_->insnSeqId_, aggrReq->pkt_->tid_);
      flushQueue_[flushReq->getID()] = writeReq;
      requestQueue_.push({flushReq, 0});
    } else {
  ...
  for (int x = 0; x < numCacheLinesNeeded; x++) {
    ...
    if (aggrReq->pkt_->isAtomic()) {
      readReq = new StandardMem::LoadLink(pAddrStart, currReqSize, 0,
                                          ...
                                          aggrReq->pkt_->tid_);
      if (print)
        std::cout << "\t\tAtomic read access at " << std::hex << vAddrStart
                  << std::dec << " from instruction "
                  << aggrReq->pkt_->insnSeqId_ << " with id "
                  << readReq->getID() << std::endl;
    } else {
      // uint32_t flags = 0;
      // if (aggrReq->pkt_->isStoreAddr()) flags = 0x00010000;
...
void SimEngMemInterface::tick() {
  ...
  while (reqsInFlight_ < 1024 && !requestQueue_.empty()) {
    if (requestQueue_.front().second)
      ...
      if (print)
        std::cout << requestQueue_.front().first->getString() << " @ "
                  << tickCounter_ << std::endl;
...
void SimEngMemInterface::SimEngMemHandlers::handle(StandardMem::ReadResp* rsp) {
  ...
  if (itrLat != memInterface_.idTracking_.end()) {
    ...
    if (memInterface_.latLoadMap_.find(lat) == memInterface_.latLoadMap_.end())
      memInterface_.latLoadMap_[lat] = 1;
    else
      memInterface_.latLoadMap_[lat]++;
    // if (print) {
...
void SimEngMemInterface::SimEngMemHandlers::handle(
  ...
  if (itrLat != memInterface_.idTracking_.end()) {
    uint64_t lat = memInterface_.tickCounter_ - memInterface_.idTracking_[id];
    if (memInterface_.latStoreMap_.find(lat) ==
        memInterface_.latStoreMap_.end())
      memInterface_.latStoreMap_[lat] = 1;
    else
      memInterface_.latStoreMap_[lat]++;
    // if (print) {
    //   memInterface_.outputFile_
    //       << reinterpret_cast<SimEngMemInterface::AggregateWriteRequest*>(
    //              itr->second)
  ...
  if (aggrReq->pkt_->isAtomic())
    if (print)
      std::cout << "\t\tAtomic write access at " << std::hex
                << aggrReq->pkt_->vaddr_ << std::dec << " from instruction "
                << aggrReq->pkt_->insnSeqId_ << " returned with failure of "
                << failure << std::endl;
  if (failure) {
...
void SimEngMemInterface::SimEngMemHandlers::handle(
  ...
  // Send off the associated write request
  auto itrFlush = memInterface_.flushQueue_.find(id);
  if (itrFlush != memInterface_.flushQueue_.end()) {
    if (print)
      std::cout << itrFlush->second->getString() << " @ "
                << memInterface_.tickCounter_ << std::endl;
    memInterface_.dataMem_->send(itrFlush->second);
    memInterface_.idTracking_[itrFlush->second->getID()] =
        memInterface_.tickCounter_;
    memInterface_.reqsInFlight_++;
    memInterface_.flushQueue_.erase(itrFlush);
  }
...
void SimEngMemInterface::printLatencies() const {
  ...
  uint64_t numL1DMisses = 0;
  for (auto const& x : latLoadMap_) {
    if (x.first >= 40) numL1DMisses += x.second;
    if (orderedLats.size()) {
      bool inserted = 0;
      auto it = orderedLats.begin();
      for (; it < orderedLats.end(); it++) {
        if (x.second > latLoadMap_.at(*it)) {
      ...
      if (orderedLats.size() > 100) orderedLats.pop_back();
    } else
      orderedLats.push_back(x.first);
  }
  std::cout << "[SSTSimEng::SimEngMemInterface] " << dataMem_->getName()
            << "Proposed LOAD L1D misses " << numL1DMisses << std::endl;
  for (auto& x : orderedLats) {
    std::cout << "[SSTSimEng::SimEngMemInterface] " << dataMem_->getName()
              << "Memory load latency of " << x << " occured "
              << latLoadMap_.at(x) << " times" << std::endl;
  }

  orderedLats = {};
  numL1DMisses = 0;
  for (auto const& x : latStoreMap_) {
    if (x.first >= 40) numL1DMisses += x.second;
    if (orderedLats.size()) {
      bool inserted = 0;
      auto it = orderedLats.begin();
      for (; it < orderedLats.end(); it++) {
        if (x.second > latStoreMap_.at(*it)) {
          orderedLats.insert(it, x.first);
          inserted = true;
          break;
        }
  ...
  std::cout << "[SSTSimEng::SimEngMemInterface] " << dataMem_->getName()
            << "Proposed STORE L1D misses " << numL1DMisses << std::endl;
  for (auto& x : orderedLats) {
    std::cout << "[SSTSimEng::SimEngMemInterface] " << dataMem_->getName()
              << "Memory store latency of " << x << " occured "
              << latStoreMap_.at(x) << " times" << std::endl;
  }
}