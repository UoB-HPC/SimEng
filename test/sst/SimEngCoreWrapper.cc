...
SimEngCoreWrapper::SimEngCoreWrapper(SST::ComponentId_t id, SST::Params& params)
    : SST::Component(id), fromCacheListener(nullptr) {
  ...
  // Instantiate the cache listener port
  output_.verbose(CALL_INFO, 1, 0, "Configure cache listener link\n");
  fromCacheListener =
      configureLink(std::string("from_cache_listener_link"), "1ps",
                    new Event::Handler<SimEngCoreWrapper>(
                        this, &SimEngCoreWrapper::recvCacheListenerEvent));
  if (!fromCacheListener) {
    output_.fatal(
        CALL_INFO, -1,
        "Did not find any connected links on port from_cache_listener_link\n");
  }

  // Instantiate the cache prefetcher port
  output_.verbose(CALL_INFO, 1, 0, "Configure cache prefetcher link\n");
  fromCachePrefetcher =
      configureLink(std::string("from_prefetcher_link"), "1ps",
                    new Event::Handler<SimEngCoreWrapper>(
                        this, &SimEngCoreWrapper::recvCachePrefetcherEvent));
  if (!fromCachePrefetcher) {
    output_.fatal(
        CALL_INFO, -1,
        "Did not find any connected links on port from_prefetcher_link\n");
  }
...
void SimEngCoreWrapper::finish() {
  ...
  std::cout << std::endl;
  std::cout << "[SimEng] Finished in ";
  std::cout << (duration / 3600000) << "hr:";
  std::cout << (duration / 60000) % 60 << "m:";
  std::cout << (duration / 1000) % 60 << "s:";
  std::cout << duration % 100 << "ms" << std::endl;
}

bool SimEngCoreWrapper::clockTick(SST::Cycle_t current_cycle) {
  // std::cerr << "--- " << current_cycle << " ---" << std::endl;
...
void SimEngCoreWrapper::fabricateSimEngCore() {
  ...
  mmu_ = std::make_shared<simeng::memory::MMU>(
      [this](uint64_t addr, uint64_t pid, bool reverse) -> uint64_t {
        return translateVAddr(addr, pid, reverse);
      },
      [this]() { return signalPFQClear(); });
    ...
    case PacketType::Translate: {
      ...
      if (simeng::OS::masks::faults::getFaultCode(paddr) ==
        ...
        fakeTLB_[alignedVaddr] =
            downAlign(paddr, simeng::OS::defaults::PAGE_SIZE);
        // reverseFakeTLB_[downAlign(paddr, simeng::OS::defaults::PAGE_SIZE)] =
        //     alignedVaddr;

        // uint64_t setAssoc = alignedVaddr & 0xfffff;
        // l2TLBSetAssoc[setAssoc].push_back(alignedVaddr);
        // while (l2TLBSetAssoc[setAssoc].size() > 4) {
        //   fakeTLB_.erase(l2TLBSetAssoc[setAssoc].front());
        //   l2TLBSetAssoc[setAssoc].pop_front();
        // }

        // if (std::find(l2TLB_LRU.begin(), l2TLB_LRU.end(), alignedVaddr) ==
        //     l2TLB_LRU.end())
        //   l2TLB_LRU.push_back(alignedVaddr);
        // while (fakeTLB_.size() > 1024) {
        //   fakeTLB_.erase(l2TLB_LRU.front());
        //   l2TLB_LRU.pop_front();
        // }
...
uint64_t SimEngCoreWrapper::translateVAddr(uint64_t addr, uint64_t pid,
                                           bool reverse) {
  // if (reverse) {
  //   uint64_t alignedPaddr = downAlign(addr, simeng::OS::defaults::PAGE_SIZE);
  //   if (reverseFakeTLB_.find(alignedPaddr) != fakeTLB_.end()) {
  //     return (addr & (simeng::OS::defaults::PAGE_SIZE - 1)) +
  //            reverseFakeTLB_.at(alignedPaddr);
  //   }
  //   return -1;
  // }
  // std::cerr << "Translating " << std::hex << vaddr << std::dec << std::endl;
  uint64_t alignedVaddr = downAlign(addr, simeng::OS::defaults::PAGE_SIZE);
  ...
  if (fakeTLB_.find(alignedVaddr) != fakeTLB_.end()) {
    ...
    translation->setVirtualAddr(addr, pid);
    // if (vaddr == 0x1fd68) {
...
void SimEngCoreWrapper::signalPFQClear() {
  // output_.verbose(CALL_INFO, 0, 0, "\tSignalling PRF clear\n");
  ControlEvent* cntEv = new ControlEvent(true);
  fromCachePrefetcher->send(new ControlEvent(cntEv));
  delete cntEv;
}
  ...
  if (sysInfo->getPayload().syscallId == 215 ||
    ...
    for (size_t i = 0;
      ...
      if (it != fakeTLB_.end()) {
        ...
        // auto itR = reverseFakeTLB_.find(it->second);
        // if (itR != reverseFakeTLB_.end()) {
        //   reverseFakeTLB_.erase(itR);
        // }
        // if (l2TLBSetAssoc.find(it->first & 0xffffff) != l2TLBSetAssoc.end())
        // {
        //   l2TLBSetAssoc.erase(it->first & 0xffffff);
        // }
        fakeTLB_.erase(it);
      }
    }
...
void SimEngCoreWrapper::recvCacheListenerEvent(Event* ev) {
  CacheListenerEvent* event = dynamic_cast<CacheListenerEvent*>(ev);
  MemHierarchy::NotifyResultType resType = event->getResultType();
  MemHierarchy::NotifyAccessType accType = event->getAccessType();

  // output_.verbose(CALL_INFO, 0, 0,
  //                 "Recieved cache access event - size: %d, targAddr: %"
  //                 PRIx64
  //                 ", physAddr: %" PRIx64 ", virtAddr: %" PRIx64
  //                 ", instPtr: %" PRIx64 ", access: %d, result: %d\n",
  //                 event->getSize(), event->getTargetAddress(),
  //                 event->getPhysicalAddress(), event->getVirtualAddress(),
  //                 event->getInstructionPointer(), event->getAccessType(),
  //                 event->getResultType());

  // std::cerr << "[SimEng]\tCache Event ";
  // if (accType == MemHierarchy::NotifyAccessType::READ)
  //   std::cerr << "READ ";
  // else if (accType == MemHierarchy::NotifyAccessType::WRITE)
  //   std::cerr << "WRITE ";
  // else if (accType == MemHierarchy::NotifyAccessType::EVICT)
  //   std::cerr << "EVICT ";
  // else if (accType == MemHierarchy::NotifyAccessType::PREFETCH)
  //   std::cerr << "PREFETCH ";
  // if (resType == MemHierarchy::NotifyResultType::HIT)
  //   std::cerr << "HIT ";
  // else if (resType == MemHierarchy::NotifyResultType::MISS)
  //   std::cerr << "MISS ";
  // else if (resType == MemHierarchy::NotifyResultType::NA)
  //   std::cerr << "NA ";

  // std::cerr << "at vaddr " << std::hex << event->getVirtualAddress() <<
  // std::dec
  //           << std::endl;

  if (resType == MemHierarchy::NotifyResultType::MISS &&
      (accType == MemHierarchy::NotifyAccessType::READ ||
       accType == MemHierarchy::NotifyAccessType::WRITE ||
       accType == MemHierarchy::NotifyAccessType::PREFETCH)) {
    uint64_t vAddr = event->getVirtualAddress();
    mmu_->markMiss(vAddr, accType);
  }

  delete event;
}

void SimEngCoreWrapper::recvCachePrefetcherEvent(Event* ev) {
  PrefetchEvent* event = dynamic_cast<PrefetchEvent*>(ev);
  uint32_t size = event->getSize();
  uint64_t paddr = event->getPAddr();
  uint64_t vaddr = event->getVAddr();

  if (vaddr == -1) return;

  // output_.verbose(CALL_INFO, 0, 0,
  //                 "Recieved prefetch access event - size: %d, prfAddr: %"
  //                 PRIx64
  //                 "\n",
  //                 size, addr);
  mmu_->notifyPrefetch(paddr, vaddr, size);
