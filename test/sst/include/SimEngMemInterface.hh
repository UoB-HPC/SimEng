...
class SimEngMemInterface : public simeng::memory::Mem {
 public:
  ...
  class SimEngMemHandlers : public StandardMem::RequestHandler {
   ...
   public:
     ...
     * overriden to return atomic access success or delete.
     */
    void handle(StandardMem::WriteResp* resp) override;

    /**
     * Overloaded instance of handle method to handle flush request responses
     * overriden to delete the incoming responses as SimEng does not have any
     * use for it.
     */
    void handle(StandardMem::FlushResp* resp) override;
 ...
 private:
  ...
  std::unordered_map<uint64_t, uint64_t> latLoadMap_;
  std::unordered_map<uint64_t, uint64_t> latStoreMap_;

  std::queue<std::pair<StandardMem::Request*, bool>> requestQueue_;

  std::unordered_map<uint64_t, StandardMem::Request*> flushQueue_;
