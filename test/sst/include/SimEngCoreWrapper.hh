...
class SimEngCoreWrapper : public SST::Component {
 public:
  ...
  SST_ELI_DOCUMENT_STATISTICS(
      ...
      {"issue.portBusyStalls",
       ...
       "count", 1},
      {"branch.executed", "Number of branches executed", "count", 1},
      {"branch.mispredict", "Number of branches mispredicted", "count", 1},
      {"branch.missrate", "Branch missrate", "count", 1},
      {"lsq.loadViolations",
      ...
      {"idle.ticks",
       ...
       "count", 1},
      {"exception.ticks",
       "Number of ticks in which a core has an active exception", "count", 1},
      {"context.switches", "Number of context switches a core undergoes",
       "count", 1},
      {"rob.numLoadsRetired", "Number of loads retired", "count", 1},
      {"rob.numStoresRetired", "Number of stores retired", "count", 1},
      {"mmu.numInsnReads", "Number of instruction reads handled by the MMU",
       "count", 1},
      {"mmu.numDataReads", "Number of data reads handled by the MMU", "count",
       1},
      {"mmu.numDataWrites", "Number of data writes handled by the MMU", "count",
       1},
  ...
  SST_ELI_DOCUMENT_PORTS({"from_cache_listener_link",
                          "Link to a cache listener",
                          {"sstsimeng.CacheListenerEvent"}},
                         {"from_prefetcher_link",
                          "Link to a cache prefetcher",
 ...
 private:
  ...
  uint64_t translateVAddr(uint64_t addr, uint64_t pid, bool reverse = false);
  ...
  void recvCacheListenerEvent(Event* ev);

  void recvCachePrefetcherEvent(Event* ev);
  ...
  std::map<std::string, Statistic<uint64_t>*> statsU64 = {
      ...
      {"fetch.fetchStalls", registerStatistic<uint64_t>("fetch.fetchStalls")},
      {"decode.earlyFlushes",
       registerStatistic<uint64_t>("decode.earlyFlushes")},
      {"rename.allocationStalls",
       registerStatistic<uint64_t>("rename.allocationStalls")},
      {"rename.robStalls", registerStatistic<uint64_t>("rename.robStalls")},
      {"rename.lqStalls", registerStatistic<uint64_t>("rename.lqStalls")},
      {"rename.sqStalls", registerStatistic<uint64_t>("rename.sqStalls")},
      {"dispatch.rsStalls", registerStatistic<uint64_t>("dispatch.rsStalls")},
      {"issue.frontendStalls",
       registerStatistic<uint64_t>("issue.frontendStalls")},
      {"issue.backendStalls",
       registerStatistic<uint64_t>("issue.backendStalls")},
      {"issue.portBusyStalls",
       registerStatistic<uint64_t>("issue.portBusyStalls")},
      {"branch.executed", registerStatistic<uint64_t>("branch.executed")},
      {"branch.mispredict", registerStatistic<uint64_t>("branch.mispredict")},
      {"lsq.loadViolations", registerStatistic<uint64_t>("lsq.loadViolations")},
      {"idle.ticks", registerStatistic<uint64_t>("idle.ticks")},
      {"exception.ticks", registerStatistic<uint64_t>("exception.ticks")},
      {"context.switches", registerStatistic<uint64_t>("context.switches")},
      {"rob.numLoadsRetired",
       registerStatistic<uint64_t>("rob.numLoadsRetired")},
      {"rob.numStoresRetired",
       registerStatistic<uint64_t>("rob.numStoresRetired")},
      {"mmu.numInsnReads", registerStatistic<uint64_t>("mmu.numInsnReads")},
      {"mmu.numDataReads", registerStatistic<uint64_t>("mmu.numDataReads")},
      {"mmu.numDataWrites", registerStatistic<uint64_t>("mmu.numDataWrites")},
      {"duration", registerStatistic<uint64_t>("duration")}};

  std::map<std::string, Statistic<float>*> statsF32 = {
      {"ipc", registerStatistic<float>("ipc")},
      {"branch.missrate", registerStatistic<float>("branch.missrate")},
  ...
  std::unordered_map<uint64_t, uint64_t> fakeTLB_;
  //   std::unordered_map<uint64_t, uint64_t> reverseFakeTLB_;
  //   std::unordered_map<uint64_t, std::deque<uint64_t>> l2TLBSetAssoc;
  ...
  Link* fromCacheListener;
  Link* fromCachePrefetcher;
};  // namespace SSTSimEng
