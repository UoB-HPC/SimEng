...
PerceptronPredictor::PerceptronPredictor(ryml::ConstNodeRef config)
    : btbBits_(config::SimInfo::getValue<uint64_t>(
      ...
      globalHistoryLength_(config::SimInfo::getValue<uint64_t>(
          config["Branch-Predictor"]["Global-History-Length"])),
      rasSize_(config::SimInfo::getValue<uint64_t>(
          config["Branch-Predictor"]["RAS-entries"])) {
...
BranchPrediction PerceptronPredictor::predict(uint64_t address, BranchType type,
  ...
  // if (address == 0xffffdefc75f4) {
  // std::cerr << "[SimEng]\t\thashedIndex = " << std::hex << hashedIndex
  //           << std::dec << " | perceptron = ";
  // for (int i = 0; i < perceptron.size(); i++)
  //   std::cerr << signed(perceptron[i]) << " ";
  // std::cerr << "| globalHistory_ = " << std::hex << globalHistory_ <<
  // std::dec
  //           << " | Pout = " << Pout << std::endl;
  // }
  ...
  touched_[address] = prediction;

  return prediction;
}

BranchPrediction PerceptronPredictor::predict(uint64_t address) {
  BranchPrediction prediction = {false, 0};

  if (touched_.find(address) != touched_.end()) {
    prediction = touched_[address];
  }
...
void PerceptronPredictor::update(uint64_t address, bool isTaken,
  ...
  // std::cerr << "[SimEng]\tUpdating entry " << std::hex << address << std::dec
  //           << " - " << instructionId << " | " << std::hex << hashedIndex
  //           << std::dec << " | prevPout = " << prevPout
  //           << " | prevGlobalHistory = " << std::hex << prevGlobalHistory
  //           << std::dec << std::endl;
  ...
  if ((directionPrediction != isTaken) ||
    ...
    // std::cerr << "[SimEng]\t\tSeen as wrong" << std::endl;
    int8_t t = (isTaken) ? 1 : -1;
    // std::cerr << "[SimEng]\t\tt = " << signed(t) << std::endl;
