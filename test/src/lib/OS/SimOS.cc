...
uint64_t SimOS::createProcess(span<char> instructionBytes) {
  ...
  auto sendToMem = [this](std::vector<char> data, uint64_t pAddr,
    ...
    // std::cerr << "Wrote Proc image addr " << std::hex << vAddr << std::dec
...
uint64_t SimOS::handleVAddrTranslation(uint64_t addr, uint64_t tid,
                                       bool reverse) {
  // Top-byte ignore (TBI): https://en.wikichip.org/wiki/arm/tbi
  uint64_t vaddr = addr & 0x00ffffffffffffffull;
  ...
  uint64_t tAddr = process->handlePageFault(vaddr);
  faultCode = masks::faults::getFaultCode(tAddr);
...
VAddrTranslator SimOS::getVAddrTranslator() {
  auto fn = [this](uint64_t addr, uint64_t pid, bool reverse) -> uint64_t {
    return handleVAddrTranslation(addr, pid, reverse);
  };
  return fn;
}
