...
void ModelConfig::setExpectations(bool isDefault) {
  ...
  expectations_["Process-Image"].addChild(
      ExpectationNode::createExpectation<uint64_t>(200000, "Mmap-Size"));
  expectations_["Process-Image"]["Mmap-Size"].setValueBounds<uint64_t>(
      1, UINT64_MAX);

  // Early check on ["Process-Image"][wildcard] as values are needed to inform
  // the expected lower bound of the ["Memory-Hierarchy"]["DRAM"]["Size"] value
  uint64_t simMemLowerBound = 500000;
  if (!isDefault) {
    simMemLowerBound = 0;
    uint64_t regionSize = 0;
    std::vector<std::string> subFields = {"Heap-Size", "Stack-Size",
                                          "Mmap-Size"};
    // Get the size of each process image region to determine the lower bound of
    // the simulated memory size
    for (const auto& field : subFields) {
      ValidationResult result =
          expectations_["Process-Image"][field].validateConfigNode(
              configTree_["Process-Image"][ryml::to_csubstr(field)]);
      configTree_["Process-Image"][ryml::to_csubstr(field)] >> regionSize;
      if (!result.errored) {
        simMemLowerBound += regionSize;
      } else {
        std::cerr
            << "[SimEng:ModelConfig] Invalid \"" << field << "\" value of \""
            << regionSize << "\" passed in config file due to \""
            << result.message
            << "\" error. Cannot continue with config validation. Exiting."
            << std::endl;
        exit(1);
  ...
  expectations_["Pipeline-Widths"].addChild(
      ExpectationNode::createExpectation<uint64_t>(1, "FetchWidth"));
  expectations_["Pipeline-Widths"]["FetchWidth"].setValueBounds<uint64_t>(
      1, UINT16_MAX);
  ...
  // Memory-Hierarchy
  expectations_.addChild(
      ExpectationNode::createExpectation("Memory-Hierarchy"));

  expectations_["Memory-Hierarchy"].addChild(
      ExpectationNode::createExpectation<uint64_t>(64, "Cache-Line-Width"));
  expectations_["Memory-Hierarchy"]["Cache-Line-Width"]
      .setValueBounds<uint64_t>(1, UINT16_MAX);

  // Early check on ["Memory-Hierarchy"]["Cache-Line-Width"] as values are
  // needed to inform the expected lower bound of the
  // ["Memory-Hierarchy"]["L1-Data"]["Size"] value
  uint64_t cacheLineWidth = 64;
  if (!isDefault) {
    ValidationResult result =
        expectations_["Memory-Hierarchy"]["Cache-Line-Width"]
            .validateConfigNode(
                configTree_["Memory-Hierarchy"]["Cache-Line-Width"]);
    configTree_["Memory-Hierarchy"]["Cache-Line-Width"] >> cacheLineWidth;
    if (result.errored) {
      std::cerr << "[SimEng:ModelConfig] Invalid Cache-Line-Width value of \""
                << cacheLineWidth << "\" passed in config file due to \""
                << result.message
                << "\" error. Cannot continue with config validation. Exiting."
                << std::endl;
    }
  ...
  expectations_["Memory-Hierarchy"].addChild(
      ExpectationNode::createExpectation("L1-Data"));

  expectations_["Memory-Hierarchy"]["L1-Data"].addChild(
      ExpectationNode::createExpectation<uint64_t>(1, "Access-Latency"));
  expectations_["Memory-Hierarchy"]["L1-Data"]["Access-Latency"]
      .setValueBounds<uint64_t>(1, UINT16_MAX);

  expectations_["Memory-Hierarchy"]["L1-Data"].addChild(
      ExpectationNode::createExpectation<uint64_t>(32768, "Size"));
  expectations_["Memory-Hierarchy"]["L1-Data"]["Size"].setValueBounds<uint64_t>(
      cacheLineWidth, UINT64_MAX);

  expectations_["Memory-Hierarchy"].addChild(
      ExpectationNode::createExpectation("DRAM"));

  expectations_["Memory-Hierarchy"]["DRAM"].addChild(
      ExpectationNode::createExpectation<uint64_t>(1, "Access-Latency"));
  expectations_["Memory-Hierarchy"]["DRAM"]["Access-Latency"]
      .setValueBounds<uint64_t>(1, UINT16_MAX);

  expectations_["Memory-Hierarchy"]["DRAM"].addChild(
      ExpectationNode::createExpectation<uint64_t>(104857600, "Size"));
  expectations_["Memory-Hierarchy"]["DRAM"]["Size"].setValueBounds<uint64_t>(
      simMemLowerBound, UINT64_MAX);

  // LSQ-Memory-Interface
  expectations_.addChild(
      ExpectationNode::createExpectation("LSQ-Memory-Interface"));

  expectations_["LSQ-Memory-Interface"].addChild(
      ExpectationNode::createExpectation<bool>(false, "Exclusive"));
  expectations_["LSQ-Memory-Interface"]["Exclusive"].setValueSet(
      std::vector{false, true});

  expectations_["LSQ-Memory-Interface"].addChild(
      ExpectationNode::createExpectation<uint64_t>(32, "Load-Bandwidth"));
  expectations_["LSQ-Memory-Interface"]["Load-Bandwidth"]
      .setValueBounds<uint64_t>(1, UINT16_MAX);

  expectations_["LSQ-Memory-Interface"].addChild(
      ExpectationNode::createExpectation<uint64_t>(32, "Store-Bandwidth"));
  expectations_["LSQ-Memory-Interface"]["Store-Bandwidth"]
      .setValueBounds<uint64_t>(1, UINT16_MAX);

  expectations_["LSQ-Memory-Interface"].addChild(
      ExpectationNode::createExpectation<uint64_t>(
          1, "Permitted-Requests-Per-Cycle"));
  expectations_["LSQ-Memory-Interface"]["Permitted-Requests-Per-Cycle"]
      .setValueBounds<uint64_t>(1, UINT16_MAX);

  expectations_["LSQ-Memory-Interface"].addChild(
      ExpectationNode::createExpectation<uint64_t>(
          1, "Permitted-Loads-Per-Cycle"));
  expectations_["LSQ-Memory-Interface"]["Permitted-Loads-Per-Cycle"]
      .setValueBounds<uint64_t>(1, UINT16_MAX);

  expectations_["LSQ-Memory-Interface"].addChild(
      ExpectationNode::createExpectation<uint64_t>(
          1, "Permitted-Stores-Per-Cycle"));
  expectations_["LSQ-Memory-Interface"]["Permitted-Stores-Per-Cycle"]
      .setValueBounds<uint64_t>(1, UINT16_MAX);
  ...
  expectations_["CPU-Info"].addChild(
      ExpectationNode::createExpectation<std::string>("0x00", "CPU-Implementer",
                                                      true));
  ...
  expectations_["CPU-Info"].addChild(
      ExpectationNode::createExpectation<std::string>("0x000", "CPU-Part",
                                                      true));
  ...
  expectations_.addChild(ExpectationNode::createExpectation<std::string>(
      "", "Environment-Variables", true));
  expectations_["Environment-Variables"].setAsSequence();
}
