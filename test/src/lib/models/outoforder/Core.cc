...
Core::Core(arch::Architecture& isa, BranchPredictor& branchPredictor,
    ...
    : isa_(isa),
      ...
      mmu_(mmu),
      fetchToDecodeBuffer_(config::SimInfo::getValue<int>(
                               config["Pipeline-Widths"]["FetchWidth"]),
                           {}),
      ...
      reorderBuffer_(
          ...
          [this](auto reg, auto val) {
            dispatchIssueUnit_.updateScoreboard(reg, val);
          },
  ...
  for (size_t i = 0; i < config["Execution-Units"].num_children(); i++) {
    ...
    executionUnits_.emplace_back(
        i, issuePorts_[i], completionSlots_[i],
        [this](auto regs, auto values, auto producerGroup) {
          dispatchIssueUnit_.forwardOperands(regs, values, producerGroup);
        },
        [this](auto uop) { loadStoreQueue_.startLoad(uop); },
        [this](auto uop) { loadStoreQueue_.supplyStoreInfo(uop); },
...
void Core::tick() {
  ticks_++;
  // std::cerr << "[SimEng] === " << ticks_ << " ===" << std::endl;
  if (print) {
    // outputFile_ << "==== " << ticks_ << " ====" << std::endl;
    // outputFile2_ << "==== " << ticks_ << " ====" << std::endl;
  }
  subTicks_++;

  if (subTicks_ > 100000000) {
    ...
    std::cout << "[SimEng:Core" << coreId_ << ":TID" << currentTID_
              ...
              << (committed - subInsns_)
              << " instructions) with last retired address being 0x" << std::hex
              << reorderBuffer_.getLastAddr() << std::dec << ":"
              << reorderBuffer_.getLastSeqId() << std::endl;
    subTicks_ = 0;
    subInsns_ = committed;
  }
  ...
  procTicks_++;
  // std::cerr << "[SimEng] === " << procTicks_ << " ===" << std::endl;

  // if (mmu_->hasActivePageFault()) {
  //   idle_ticks_++;
  //   return;
  // }
  ...
  uint64_t preCommit = reorderBuffer_.getInstructionsCommittedCount();
  unsigned int numRet = reorderBuffer_.commit(commitWidth_, procTicks_);
  if (numRet & 0x0F00) {
    // mmu_->enablePrint(true);
    // loadStoreQueue_.enableAccessPrint(true);
    // loadStoreQueue_.enableSTBPrint(true);
    // loadStoreQueue_.resetStats();
    // reorderBuffer_.resetStats();
    // numCommitted_ = {};
    // dispatchIssueUnit_.enableIssuePrint(true);
    // renameUnit_.enablePrint(true);
    // fetchUnit_.enablePrint(true);
    // enablePrint_ = true;
    // portAllocator_.enablePrint(true);
    getStats();
    // dispatchIssueUnit_.resetStats();
  } else if (numRet & 0xF000) {
    mmu_->enablePrint(false);
    loadStoreQueue_.enableAccessPrint(false);
    loadStoreQueue_.enableSTBPrint(false);
    // dispatchIssueUnit_.enableForwardPrint(false);
    // getStats();
  } else {
    // if (numRet <= commitWidth_) numCommitted_[numRet]++;
    numCommitted_[reorderBuffer_.getInstructionsCommittedCount() - preCommit]++;
  }
...
void Core::flushIfNeeded() {
  ...
  if (euFlush || reorderBuffer_.shouldFlush()) {
    ...
    if (reorderBuffer_.shouldFlush() &&
      ...
      if (enablePrint_) std::cerr << "[SimEng] ROB flush" << std::endl;
    } else {
      if (enablePrint_) std::cerr << "[SimEng] EU flush" << std::endl;
    }
    // if (currentTID_ == 24)
    ...
    fetchUnit_.flushLoopBuffer();
    if (enablePrint_) std::cerr << "[SimEng] Decode flush" << std::endl;
    fetchUnit_.updatePC(targetAddress);
    fetchToDecodeBuffer_.flushBranchMacroOps(branchPredictor_);
    fetchToDecodeBuffer_.fill({});
    fetchToDecodeBuffer_.stall(false);
...
void Core::raiseException(const std::shared_ptr<Instruction>& instruction) {
  ...
  //               8}).get<uint64_t>()
  //               << " started by " << std::hex
  //               << instruction->getInstructionAddress() << std::dec
  //               << std::endl;
  //   outputFile2_
  //       << "\tSyscall "
  //       << getArchitecturalRegisterFileSet().get({0, 8}).get<uint64_t>()
  //       << " started by " << std::hex << instruction->getInstructionAddress()
  //       << std::dec << "(" << instruction->getSequenceId() << ")" <<
  //       std::endl;
  // }
}
...
void Core::processException() {
  ...
  if (result.fatal) {
    ...
    if (enablePrint_) std::cerr << "[SimEng] Syscall flush" << std::endl;
...
void Core::applyStateChange(const OS::ProcessStateChange& change) {
  ...
  for (size_t i = 0; i < change.memoryAddresses.size(); i++) {
    ...
    // loadStoreOutputFile_ << "core:" << std::hex
    //                      << change.memoryAddresses[i].vaddr << std::dec <<
    //                      "("
    //                      << change.memoryAddresses[i].size << ")|" << ticks_
    //                      << std::endl;
...
std::map<std::string, std::string> Core::getStats() const {
  ...
  auto allocationStallsVec = renameUnit_.getAllocationStallsVec();
  ...
  auto torStalls = renameUnit_.getTORStalls();
  ...
  std::map<std::string, std::string> stats = {
      ...
      {"rename.torStalls", FormatWithCommas<uint64_t>(torStalls)},
      ...
      {"lsq.STBQuietDrains",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getSTBQuietDrains())},
      {"lsq.STBCapacityDrains",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getSTBCapacityDrains())},
      {"lsq.STBMismatchDrains",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getSTBMismatchDrains())},
      {"lsq.STBSystemDrains",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getSTBSystemDrains())},
      {"lsq.LoadReqs",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getLoadReqs())},
      {"lsq.SQSupplies",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getSQSupplies())},
      {"lsq.Conflicts",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getConflicts())},
      {"lsq.LoadTLBResubs",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getLoadTLBResubs())},
      {"lsq.LoadDataResubs",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getLoadDataResubs())},
      {"lsq.StoreAddrReqs",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getStoreAddrReqs())},
      {"lsq.StoreAddrResubs",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getStoreAddrResubs())},
      {"lsq.StoreDataReqs",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getStoreDataReqs())},
      {"lsq.StoreDataResubs",
       FormatWithCommas<uint64_t>(loadStoreQueue_.getStoreDataResubs())},
      // {"mmu.L1DTLBMisses",
      //  FormatWithCommas<uint64_t>(mmu_->getNumL1DTLBMisses())},
      // {"mmu.TLBMSHRBlocks",
      //  FormatWithCommas<uint64_t>(mmu_->getNumTLBMSHRBlocks())},
      {"mmu.L1DDataMisses",
       FormatWithCommas<uint64_t>(mmu_->getNumL1DDataMisses())},
      {"mmu.CacheMSHRBlocks",
       FormatWithCommas<uint64_t>(mmu_->getNumCacheMSHRBlocks())}};

  for (int i = 0; i < allocationStallsVec.size(); i++) {
    std::ostringstream key;
    key << "rename.allocationStall." << i << "";
    std::ostringstream val;
    val << allocationStallsVec[i];
  ...
  const std::vector<uint64_t> missedIssues =
      dispatchIssueUnit_.getMissedIssues();
  for (int i = 0; i < missedIssues.size(); i++) {
    std::ostringstream key;
    key << "issue.Port" << i << ".missedIssues";
    std::ostringstream val;
    val << FormatWithCommas<uint64_t>(missedIssues[i]);
    stats[key.str()] = val.str();
  }

  const std::unordered_map<uint64_t, uint64_t> waitingOn =
      reorderBuffer_.getWaitingOn();
  ...
  std::unordered_map<uint64_t, uint64_t> latMap = loadStoreQueue_.getLatMap();
  std::vector<uint64_t> orderedLats = {};
  for (auto const& x : latMap) {
    if (orderedLats.size()) {
      bool inserted = 0;
      auto it = orderedLats.begin();
      for (; it < orderedLats.end(); it++) {
        if (x.second > latMap.at(*it)) {
          orderedLats.insert(it, x.first);
          inserted = true;
          break;
        }
      }
      if (!inserted) orderedLats.push_back(x.first);
      if (orderedLats.size() > 10) orderedLats.pop_back();
    } else
      orderedLats.push_back(x.first);
  }
  for (auto& x : orderedLats) {
    std::ostringstream key;
    key << "lsq.latency." << x;
    std::ostringstream val;
  ...
  std::unordered_map<uint64_t, uint64_t> inflightReqNums =
      mmu_->getNumInFlightRequestsTally_();
  for (const auto& num : inflightReqNums) {
    std::ostringstream key;
    key << "mmu.inflight." << num.first;
    std::ostringstream val;
    val << FormatWithCommas<uint64_t>(num.second);
    stats[key.str()] = val.str();
  }

  latMap = fetchUnit_.getLats();
  orderedLats = {};
  for (auto const& x : latMap) {
    if (orderedLats.size()) {
      bool inserted = 0;
...
void Core::schedule(simeng::OS::cpuContext newContext) {
  ...
  if (print) {
    ...
    char cwd[size];
    getcwd(cwd, size);
    str << cwd << "/thread." << currentTID_ << ".retire.out";
    outputFile_.open(str.str(), std::ofstream::out | std::ofstream::app);

    outputFile2_.close();
    std::ostringstream str2;
    str2 << cwd << "/thread." << currentTID_ << ".retireWithIDs.out";
  ...
  // str3 << cwd2 << "/thread." << currentTID_ << ".loadStores.out";
  // loadStoreOutputFile_.open(str3.str(), std::ofstream::out);
