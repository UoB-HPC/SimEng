...
Instruction::Instruction(const Architecture& architecture,
  ...
  totalDataSize_ = microOpInfo.totalDataSize;
...
std::string Instruction::getMnemonic() const {
  return std::string(metadata.mnemonic) + " " + metadata.operandStr;
}
...
std::vector<uint16_t> Instruction::getSupportedPorts() {
