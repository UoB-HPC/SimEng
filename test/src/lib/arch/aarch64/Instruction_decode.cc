...
void Instruction::decode() {
  ...
  for (size_t i = 0; i < metadata.operandCount; i++) {
    ...
    if (op.type == ARM64_OP_REG) {  // Register operand
      ...
      if (op.access & cs_ac_type::CS_AC_READ) {
        ...
        if (op.shift.value > 0)
          insnTypeMetadata &= !isNoShiftMask;  // Identify shift operands
      }
  ...
  if (accessesMemory) {
    ...
    if (!isMicroOp_) dataSize_ = calcDataSize(metadata.operands[0]);
    ...
    if (metadata.opcode == Opcode::AArch64_LDAXRB ||
      ...
      insnTypeMetadata |= isLoadRsrvdMask;
    }
  ...
  if ((347 <= metadata.opcode && metadata.opcode <= 366) ||
      ...
      microOpcode_ == MicroOpcode::FCVT_INT ||
      microOpcode_ == MicroOpcode::SCVT_INT) {
  ...
  if ((367 <= metadata.opcode && metadata.opcode <= 375) ||
    ...
    insnTypeMetadata |= isDivOrSqrtMask;
  }
  ...
  if (metadata.opcode == Opcode::AArch64_ADR ||
      metadata.opcode == Opcode::AArch64_ADRP ||
      metadata.opcode == Opcode::AArch64_LDRSWl ||
      metadata.opcode == Opcode::AArch64_LDRWl ||
      metadata.opcode == Opcode::AArch64_LDRXl ||
      metadata.opcode == Opcode::AArch64_BLR ||
      metadata.opcode == Opcode::AArch64_BR) {
    usesTOR_ = true;
  }
