...
Architecture::~Architecture() {
  ...
  if (trackMnemonics_) {
    ...
    char cwd[size];
    getcwd(cwd, size);
    std::ofstream mnemonicFile;
    std::ostringstream mnemonicStr;
    mnemonicStr << cwd << "/mnemonics.out";
    mnemonicFile.open(mnemonicStr.str(), std::ofstream::out);
    mnemonicFile.close();
    mnemonicFile.open(mnemonicStr.str(),
                      std::ofstream::out | std::ofstream::app);
    std::vector<std::pair<uint64_t, std::string>> mnemonicPairs;
    for (auto& it : mnemonicsSeen_) {
      mnemonicPairs.push_back(it);
    }
    std::sort(mnemonicPairs.begin(), mnemonicPairs.end(),
              [](auto& a, auto& b) { return a.first < b.first; });
    for (const auto& mnemonicPair : mnemonicPairs) {
      mnemonicFile << std::hex << mnemonicPair.first << std::dec << ": "
                   << mnemonicPair.second << std::endl;
    }
    mnemonicFile.close();
  }
}
...
uint8_t Architecture::predecode(const void* ptr, uint8_t bytesAvailable,
  ...
  if (trackMnemonics_)
    mnemonicsSeen_[instructionAddress] = iter->second.getMnemonic();
