...
RenameUnit::RenameUnit(PipelineBuffer<std::shared_ptr<Instruction>>& fromDecode,
  ...
  for (int i = 0; i < registerTypes; i++) allocationStallsVec_.push_back(0);
}

void RenameUnit::tick() {
  if (output_.isStalled()) {
    input_.stall(true);
    if (enablePrint_) std::cerr << "[SimEng] Rename Stalled" << std::endl;
    return;
  }
  ...
  for (size_t slot = 0; slot < input_.getWidth(); slot++) {
    ...
    if (reorderBuffer_.getFreeSpace() == 0) {
      input_.stall(true);
      if (enablePrint_) std::cerr << "[SimEng] ROB Stall" << std::endl;
      robStalls_++;
      return;
    }

    if (uop->isAtomic() && reorderBuffer_.size() > 0) {
      if (enablePrint_) std::cerr << "[SimEng] Atomic Pause" << std::endl;
      input_.stall(true);
      return;
    }
    ...
    if (isLoad) {
      if (lsq_.getLoadQueueSpace() == 0) {
        lqStalls_++;
        if (enablePrint_) std::cerr << "[SimEng] LQ Stall" << std::endl;
        input_.stall(true);
        return;
      }
    } else if (isStore) {
      if (lsq_.getStoreQueueSpace() == 0) {
        sqStalls_++;
        if (enablePrint_) std::cerr << "[SimEng] SQ Stall" << std::endl;
        input_.stall(true);
        return;
      }
    }
    ...
    for (const auto& reg : destinationRegisters) {
      ...
      if (freeRegistersAvailable_[reg.type] == 0) {
        ...
        input_.stall(true);
        if (enablePrint_)
          std::cerr << "[SimEng] " << unsigned(reg.type) << " Alloc Stall"
                    << std::endl;
        allocationStalls_++;
        allocationStallsVec_[reg.type]++;
        return;
      }
    ...
    // std::cerr << "\tRENAME: " << std::hex << uop->getInstructionAddress()
