...
bool requestsOverlap(memory::MemoryAccessTarget a,
  ...
  // and negate
  uint32_t aSize = std::max(a.size, (uint32_t)4);
  uint32_t bSize = std::max(b.size, (uint32_t)4);
  return !((a.vaddr + aSize) <= b.vaddr || (b.vaddr + bSize) <= a.vaddr);
}
          ...
          for (auto load : ldVec) {
            ...
            if (load->hasAllData()) {
              ...
              if (completionOrder_ == CompletionOrder::OUTOFORDER) {
                bool inserted = false;
                uint64_t insertTick = tickCounter_ + load->getLSQLatency();
                if (completedRequests_.size()) {
                  ...
                  while (itEntry != completedRequests_.end()) {
                    if ((*itEntry).second > insertTick) {
                      completedRequests_.insert(itEntry, {load, insertTick});
                      inserted = true;
                      break;
                    }
                    itEntry++;
                ...
                if (!inserted) completedRequests_.push_back({load, insertTick});
              }
            } else {
              bool inserted = false;
              if (requestLoadQueue_[tickCounter_].size()) {
...
void LoadStoreQueue::recursiveSTBaddition(
  ...
  if (storeBuffer_.find(baseAddr) == storeBuffer_.end()) {
  ...
  } else if (storeBuffer_.at(baseAddr).first.doesContain(target) !=
             stbOverlap::NONE) {
    std::vector<std::pair<memory::MemoryAccessTarget, RegisterValue>> requests =
        storeBuffer_[baseAddr].first.createRequests(stbReqIds_);
    ...
    storeBufferEntry newEntry(baseAddr, storeBufferEntryWidth_, id);
    storeBuffer_.at(baseAddr) = {newEntry, tickCounter_};
  }
...
void LoadStoreQueue::tick() {
    ...
    if (accessPrint_) {
      if (accepted != memory::requestSuccess::SUCCESS) {
        ...
        if (accepted == memory::requestSuccess::LIMIT)
          std::cerr << "LIMIT";
        else if (accepted == memory::requestSuccess::TRANSLATION)
        ...
        if (!chooseLoad) {
          ...
          if ((*itInsn)->isPrefetch()) {
            ...
            for (auto addr : addresses) {
              numLoads++;
              mmu_->requestPrefetch(addr.vaddr, addr.size);
            }
            (*itInsn)->setLSQIssue(tickCounter_);
              ...
              if (stbEntry != storeBuffer_.end()) {
                if ((*itInsn)->isLoadReserved()) {
                  ...
                  if (overlapStatus == stbOverlap::FULL) {
                    ...
                    if (stbPrint_)
                      std::cerr << "[SimEng]\t\t\tSupplying data from STB entry"
                                << std::endl;
                    stbSupplies_++;
                    ...
                    if (stbPrint_) {
                      std::cerr << "[SimEng]\t\t\tNo satisfying data in entry, "
                                   "draining entry "
                                << std::hex << baseLoadAddr << std::dec
                                << std::endl;
                    }
  ...
  // must be the next-to-retire instruction
  if (completionOrder_ == CompletionOrder::OUTOFORDER &&
    ...
    if (requestedCondStore_.first->isCondResultReady()) {
      completedRequests_.push_back(
          {requestedCondStore_.first,
           tickCounter_ + requestedCondStore_.first->getLSQLatency() - 1});
  ...
  while (load != requestedLoads_.end()) {
    if (load->second.first->getcycleMemRecv() != 0) {
      if (load->second.first->hasAllData() &&
        ...
        if (accessPrint_)
          std::cerr << "[SimEng]\t\tLoad hit @ " << tickCounter_ << " "
                    ...
                    << std::dec << " - " << load->second.first->getSequenceId()
        ...
        if (itrLat != idTracking_.end()) {
          lat = tickCounter_ - idTracking_[load->second.first->getSequenceId()];
  ...
  while (storeAddr != requestedStoreAddrs_.end()) {
    if (storeAddr->second.first->getreadTLBRet() &&
      ...
      if (completionOrder_ == CompletionOrder::OUTOFORDER) {
        ...
        bool inserted = false;
        uint64_t insertTick =
            tickCounter_ + storeAddr->second.first->getLSQLatency();
        if (completedRequests_.size()) {
          auto itEntry = completedRequests_.begin();
          while (itEntry != completedRequests_.end()) {
            if ((*itEntry).second > insertTick) {
              completedRequests_.insert(itEntry,
                                        {storeAddr->second.first, insertTick});
              inserted = true;
              break;
            }
            itEntry++;
  ...
  auto complItr = completedRequests_.begin();
  bool finishedLoad = false;
  while (complItr != completedRequests_.end() &&
         count < completionSlots_.size()) {
    ...
    if (complItr->second > tickCounter_) {
      if (accessPrint_)
        std::cerr << "[SimEng]\tWaiting to completed flow @ "
                  << complItr->second << " for " << std::hex
                  << complItr->first->getInstructionAddress() << std::dec
                  << " - " << complItr->first->getSequenceId() << " - "
                  << complItr->first->getOpcode() << std::endl;
      break;
    }
    auto& insn = complItr->first;

    // Don't process load instruction if it has been flushed
    if (insn->isFlushed()) {
      complItr = completedRequests_.erase(complItr);
      continue;
    }
    ...
    if (insn->isLoad()) {
      ...
      // if (insn->getLSQLatency() < 6) {
      //   if (tickCounter_ <= outputCooldown_) {
      //     complItr++;
      //     continue;
      //   }
      // }
      // for (int i = 1; i < completedRequests_.size(); i++) {
      //   if (completedRequests_[i].first->isLoad() &&
      //       completedRequests_[i].first->getLSQLatency() < 6 &&
      //       (completedRequests_[i].second -
      //       completedRequests_.front().second) <
      //           3) {
      //     std::cerr << "LSQ overlap between " << std::hex
      //               << insn->getInstructionAddress() << std::dec << "("
      //               << insn->getOpcode() << ") and " << std::hex
      //               << completedRequests_[1].first->getInstructionAddress()
      //               << std::dec << "("
      //               << completedRequests_[1].first->getOpcode() << ")"
      //               << std::endl;
      //   }
      // }
    }
    ...
    if (accessPrint_)
      std::cerr << "[SimEng]\tCompleted flow @ " << tickCounter_ << " (in "
                ...
                << " - " << insn->getSequenceId() << " - " << insn->getOpcode()
                << std::endl;
    // std::cerr << "[SimEng]\t" << (insn->isLoad() ? "LOAD" : "STORE")
    //           << std::endl;
    // Forward the results
    ...
    // if (insn->isLoad() && outputCooldown_ <= tickCounter_)
  ...
  // if (count >= completionSlots_.size() && completedRequests_.size())
  //   std::cerr << "Waisted slots?" << std::endl;
  // if (count > 2) std::cerr << "Over slotted? (" << count << ")" << std::endl;
}
