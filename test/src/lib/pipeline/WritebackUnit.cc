...
void WritebackUnit::tick() {
  for (size_t slot = 0; slot < completionSlots_.size(); slot++) {
    ...
    //   if (slot == 0 || slot == 3) {
    //     if (uop->getLatency() < 9) completionSlots_[slot].stall(true);
    //   }
    //   if (slot > 7) {
    //     if (uop->getLSQLatency() < 6) completionSlots_[slot].stall(true);
    //   }
    // }

    // std::cerr << "Writeback " << std::hex << uop->getInstructionAddress()
