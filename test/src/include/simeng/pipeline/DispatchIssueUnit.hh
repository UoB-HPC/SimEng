...
class DispatchIssueUnit {
 public:
  ...
  void updateScoreboard(const Register& reg, const RegisterValue& val);

  const std::vector<uint64_t> getPossibleIssues() const;

  const std::vector<uint64_t> getActualIssues() const;

  const std::vector<uint64_t> getMissedIssues() const;

  void resetStats();

  void enableForwardPrint(bool enable) { forwardPrint_ = enable; }

  void enablePortPrint(bool enable) { portPrint_ = enable; }

  void enableIssuePrint(bool enable) { issuePrint_ = enable; }
 ...
 private:
  ...
  std::vector<uint64_t> missedIssues_;

  std::vector<uint64_t> assoicatedIssues_;

  std::pair<bool, simeng::Register> lastDest_ = {false, {0, 0}};
  uint16_t lastPort_;

  std::vector<std::string> portNames_ = {"FLA", "PR",   "EXA", "FLB",
                                         "EXB", "EAGA", "EAGB"};

  bool forwardPrint_ = false;
  bool portPrint_ = false;
  bool issuePrint_ = false;
