...
class FetchUnit {
 public:
  ...
  void enablePrint(bool enable) { fetchPrint_ = enable; }

  std::unordered_map<uint64_t, uint64_t> getLats() const { return l1ILats_; }
 ...
 private:
  ...
  uint64_t pc_ = 0;
  uint64_t runAheadPC_ = 0;

  uint64_t tickcounter_ = 0;
  uint64_t pausedUntil_ = 0;

  std::pair<uint64_t, uint64_t> shortLoopTracker_ = {0, 0};
  uint64_t insnsBetweenTaken_ = 0;
  ...
  uint16_t mopQueueSize_ = 48;

  std::deque<simeng::MacroOp> mopQueue_;

  uint8_t mopCacheTagBits_ = 11;

  std::map<uint64_t, RegisterValue> mopCache_;

  std::deque<uint64_t> requestedBlocks_;

  std::unordered_map<uint64_t, uint64_t> reqBlkLats_;

  std::unordered_map<uint64_t, uint64_t> l1ILats_;

  std::deque<std::pair<uint64_t, uint64_t>> branchDelayed_;

  uint64_t requestBranchTargetBlock_ = -1;

  uint64_t requestedBlockId_ = 0;
  ...
  std::deque<std::pair<uint64_t, uint64_t>> stcp_;
  uint64_t sctpCntr_ = 0;
