...
class RenameUnit {
 public:
  ...
  std::vector<uint64_t> getAllocationStallsVec() const {
    return allocationStallsVec_;
  }
  ...
  uint64_t getTORStalls() const { return torStalls_; }
  ...
  void enablePrint(bool enable) { enablePrint_ = enable; }
 ...
 private:
  ...
  std::vector<uint64_t> allocationStallsVec_;
  ...
  uint64_t torStalls_ = 0;

  bool pauseForAtomic_ = false;

  uint64_t tid_ = 0;

  bool enablePrint_ = false;
};
