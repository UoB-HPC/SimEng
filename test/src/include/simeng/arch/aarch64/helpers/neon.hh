 ...
 * Returns correctly formatted RegisterValue. */
template <typename T, typename C, int I>
RegisterValue vecFCompareAbs(std::vector<RegisterValue>& operands,
                             std::function<bool(T, T)> func) {
  const T* n = operands[0].getAsVector<T>();
  const T* m = operands[1].getAsVector<T>();
  C out[16 / sizeof(C)] = {0};
...
/** Helper function for NEON instructions with the format `{f}mla vd,
 *  vn, vm`.
 * T represents the type of operands (e.g. for vn.2d, T = double).
 * I represents the number of elements in the output array to be updated (e.g.
 * for vd.8b I = 8).
 * Returns correctly formatted RegisterValue. */
template <typename T, int I>
RegisterValue vecMla_3vecs(std::vector<RegisterValue>& operands) {
  const T* d = operands[0].getAsVector<T>();
...
/** Helper function for SVE instructions with the format `fsqrt vd, vn`.
 * T represents the type of operands (e.g. for vn.2d, T = double).
 * I represents the number of elements in the output array to be updated (e.g.
 * for vd.8b I = 8).
 * Returns correctly formatted RegisterValue. */
template <typename T, int I>
RegisterValue vecFrecpe(std::vector<RegisterValue>& operands) {
  const T* n = operands[0].getAsVector<T>();
  T out[16 / sizeof(T)] = {0};
  for (int i = 0; i < I; i++) {
    out[i] = 1.0f / n[i];
 ...
 * Returns correctly formatted RegisterValue. */
template <typename T, int I>
RegisterValue vecUMaxV(std::vector<RegisterValue>& operands) {
  const T* n = operands[0].getAsVector<T>();
...
RegisterValue vecUshl(
  ...
  T out[16 / sizeof(T)] = {0};
  for (int i = 0; i < I; i++) {
    const int8_t shift = static_cast<int8_t>(m[i]);
    if (shift < 0)
      out[i] = n[i] >> -shift;
    else
      out[i] = n[i] << shift;
