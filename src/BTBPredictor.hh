...
/** A Branch Target Buffer based branch predictor. Keeps an internal BTB with previously seen branch target buffer addresses. */
class BTBPredictor : public BranchPredictor {
 public:
  /** Construct a BTB predictor, with 2^bits BTB entries. */
  BTBPredictor(uint8_t bits);

  /** Generate a branch prediction for the supplied instruction address. Finds the corresponding BTB entry and returns a "taken" prediction with the stored target. If no entry is found, predicts "not taken". */
  BranchPrediction predict(uint64_t instructionAddress) override;

  /** Update the BTB entry for the supplied instruction address with the taken state and targt address. */
 ...
 private:
  ...
  /** Generate a BTB lookup hash for the specified address by trimming the lowest `bits` bits. */
