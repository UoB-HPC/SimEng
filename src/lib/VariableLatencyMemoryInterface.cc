...
VariableLatencyMemoryInterface::VariableLatencyMemoryInterface(
    char* memory, size_t size, uint16_t iLatency, uint16_t fpLatency,
    uint16_t SVELatency)
    : memory_(memory),
      size_(size),
      iLatency_(iLatency),
      fpLatency_(fpLatency),
      SVELatency_(SVELatency) {}
...
void VariableLatencyMemoryInterface::requestRead(
    const MemoryAccessTarget& target, uint64_t requestId) {
...
void VariableLatencyMemoryInterface::requestWrite(
    const MemoryAccessTarget& target, const RegisterValue& data) {
  uint16_t latency = iLatency_;
  if (target.isFP) latency = fpLatency_;
