...
Core::Core(MemoryInterface& instructionMemory, MemoryInterface& dataMemory,
    ...
    : dataMemory_(dataMemory),
      ...
      loadStoreQueue_(
          4, dataMemory, {completionSlots_.data() + 1, 1},
      ...
      executeUnit_(
          ...
          [this]() { return isInterruptPending(); }, branchPredictor, false),
      interruptId_(-1) {
  ...
  maxInstrTimeout = -1;
  if (config["Core"]["EnableHaltCheck"].IsDefined() &&
      config["Core"]["EnableHaltCheck"].as<bool>()) {
    enableHaltCheck = true;
    if (config["Core"]["MaxStallCycleTimeout"].IsDefined()) {
      maxStallCycleTimeout =
          config["Core"]["MaxStallCycleTimeout"].as<uint64_t>();
    }
...
void Core::checkHalting() {
  ...
  if (((ticks_ - lastCommitTick_) > maxStallCycleTimeout)) {
    std::cout
        << std::dec
        << "[SimEng:Core] Max Pipeline stall cycle timeout reached at tick: "
        << (ticks_ - lastCommitTick_) << std::endl;
    hasHalted_ = true;
  }

  if ((ticks_ > maxSimCycleTimeout)) {
    std::cout << std::dec
              << "[SimEng:Core] Max Simulation cycle timeout reached at tick: "
              << ticks_ << std::endl;
    hasHalted_ = true;
  }
  ...
  if ((getInstructionsRetiredCount() > maxInstrTimeout)) {
    std::cout << std::dec
              << "[SimEng:Core] Max Instruction count timeout reached at tick: "
              << ticks_ << std::endl;
    hasHalted_ = true;
  }
}
...
void Core::tick() {
  ...
  //   isa_.getSystemRegisterTag(arch::riscv32::riscv_sysreg::SYSREG_INSTRRET);
  //   uint16_t sysreg_cycle =
  //   isa_.getSystemRegisterTag(arch::riscv32::riscv_sysreg::SYSREG_CYCLE);
  //   registerFileSet_.set(Register{0x2, sysreg_instrret},
  //   RegisterValue(static_cast<uint32_t>(writebackUnit_.getInstructionsWrittenCount()),
  //   4)); registerFileSet_.set(Register{0x2, sysreg_cycle},
  //   RegisterValue(static_cast<uint32_t>(ticks_), 4));
  ...
  for (std::shared_ptr<Instruction> inst : writebackUnit_.getInstsForTrace()) {
    ...
    registerFileSet_.set(
        Register{0x2, sysreg_instrret},
        RegisterValue(
            static_cast<uint32_t>(writebackUnit_.getInstructionsWrittenCount()),
            4));
  ...
  if (executeUnit_.shouldFlush()) {
    ...
    // Ensure instructions in the buffer if any are set to be flushed before
    // being removed, this helps with removing the respective dependencies if
    // any
...
void Core::handleException() {
  ...
  processExceptionHandler();
  //  isa_.updateInstrTrace(exceptionGeneratingInstruction_, &registerFileSet_,
  //  ticks_); lastCommitTick_ = ticks_;
  //  assert(removeInstrOrderQ(exceptionGeneratingInstruction_) && "Unexpected
  //  instruction at the top of inorder instr queue on exception");

  // TODO: This is not a good point to flush the pipeline if the exception is
  // not changing the PC.
...
void Core::addInstrOrderQ(const std::shared_ptr<Instruction>& insn) {
  // std::cout << std::dec << ticks_ << ": Adding instruction at address: 0x" <<
  // std::hex << insn->getInstructionAddress() << std::endl;
  inorderIQ_.push_back(insn);
}

bool Core::removeInstrOrderQ(const std::shared_ptr<Instruction>& insn) {
  if (insn == inorderIQ_.front()) {
    // std::cout << std::dec << ticks_ << ": Removing instruction at address:
    // 0x" << std::hex << insn->getInstructionAddress() << std::endl;
...
int16_t Core::isInterruptPending() {
  if (interruptId_ >= 0) {
    std::cout << std::dec
              << "[SimEng:Core] Interrupt Pending id: " << interruptId_
