...
//       width(associative),
//       mask((1 << bits) - 1),
//       btb(1 << bits, *(new std::vector<std::tuple<uint64_t, uint64_t,
//       int64_t>>(
//                          associative))),
//       pht(1 << bits, 1),
...
//   // If the branch is a subroutine return then try and use return address
//   stack if (uop->isRET() && ras.size() > 0) {
...
//   // Update btb entry
//   if (taken) std::get<0>(btb[btbIndex.first][btbIndex.second]) =
//   targetAddress; if (std::get<2>(btb[btbIndex.first][btbIndex.second]) == -1)
//   {
