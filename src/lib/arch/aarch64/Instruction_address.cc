  ...
  if (isMicroOp_) {
    switch (microOpcode_) {
      case MicroOpcode::LDR_ADDR: {
        ...
        generateContiguousAddresses(
            srcRegVals[0].get<uint64_t>() + metadata.operands[1].mem.disp, 1,
            dataSize_, addresses);
      ...
      case MicroOpcode::STR_ADDR: {
        std::vector<simeng::MemoryAccessTarget> addresses;
        generateContiguousAddresses(
            srcRegVals[0].get<uint64_t>() + metadata.operands[0].mem.disp, 1,
            dataSize_, addresses);
    ...
    switch (metadata.opcode) {
      case Opcode::AArch64_CASALW: {  // casal ws, wt, [xn|sp]
        setMemoryAddresses({{srcRegVals[2].get<uint64_t>(), 4}});
        break;
      }
      case Opcode::AArch64_CASALX: {  // casal xs, xt, [xn|sp]
        setMemoryAddresses({{srcRegVals[2].get<uint64_t>(), 8}});
        break;
      }
      ...
      case Opcode::AArch64_LD1_MXIPXX_H_D: {  // ld1d {zath.d[ws, #imm]}, pg/z,
        ...
        const uint64_t n = srcRegVals[partition_num + 2].get<uint64_t>();
        uint64_t m = 0;
        if (metadata.operands[2].mem.index)
          m = srcRegVals[partition_num + 3].get<uint64_t>() << 3;
      ...
      case Opcode::AArch64_LD1_MXIPXX_H_S: {  // ld1w {zath.s[ws, #imm]}, pg/z,
        ...
        uint64_t m = 0;
        if (metadata.operands[2].mem.index)
          m = srcRegVals[partition_num + 3].get<uint64_t>() << 2;
        setMemoryAddresses({(n + m), static_cast<uint16_t>(VL_bits / 8)});
        break;
      }
      ...
      case Opcode::AArch64_LD1RD_IMM: {  // ld1rd {zt.d}, pg/z, [xn, #imm]
        ...
        for (int i = 0; i < 4; i++) {
          if (p[i] != 0) {
            setMemoryAddresses(
                {{srcRegVals[1].get<uint64_t>() + metadata.operands[2].mem.disp,
                  8}});
            break;
          }
        }
      ...
      case Opcode::AArch64_LD1RQ_W: {  // ld1rqw {zd.s}, pg/z, [xn, xm, lsl #2]
        uint64_t addr =
            srcRegVals[1].get<uint64_t>() + (srcRegVals[2].get<uint64_t>() * 4);
        setMemoryAddresses({addr, static_cast<uint16_t>(16)});
        break;
      }
      ...
      case Opcode::AArch64_LD1Rv16b: {  // ld1r {vt.16b}, [xn]
        setMemoryAddresses({{srcRegVals[0].get<uint64_t>(), 16}});
        break;
      }
      ...
      case Opcode::AArch64_LD1Fourv4s_POST: {  // ld1 {vt1.4s, vt2.4s, vt3.4s,
                                               // vt4.4s}, [xn], <#imm|xm>
        uint64_t base = srcRegVals[0].get<uint64_t>();
        setMemoryAddresses(
      ...
      case Opcode::AArch64_LD1B: {  // ld1b {zt.b}, pg/z, [xn, xm]
        const uint64_t base = srcRegVals[1].get<uint64_t>();
        const uint64_t offset = srcRegVals[2].get<uint64_t>();
      ...
      case Opcode::AArch64_LD2D: {  // ld2d {zt1.d, zt2.d}, pg/z, [xn|sp, xm,
                                    // lsl #3]
        const uint64_t base = srcRegVals[1].get<uint64_t>();
        uint64_t offset = srcRegVals[2].get<uint64_t>();
      ...
      case Opcode::AArch64_LD2Twov4s_POST: {  // ld2 {vt1.4s, vt2.4s}, [xn],
                                              // #imm
        const uint64_t base = srcRegVals[2].get<uint64_t>();
        setMemoryAddresses({{base, 16}, {base + 16, 16}});
        break;
      }
      ...
      case Opcode::AArch64_LDARB: {  // ldarb wt, [xn]
        setMemoryAddresses({{srcRegVals[0].get<uint64_t>(), 1}});
        break;
      }
      ...
      case Opcode::AArch64_LDRBBroW: {  // ldrb wt,
                                        //  [xn, wm{, extend {#amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[1].get<uint32_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[0].get<uint64_t>() + offset, 1}});
        break;
      }
      case Opcode::AArch64_LDRBBroX: {  // ldrb wt,
                                        //  [xn, xm{, extend {#amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[1].get<uint64_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[0].get<uint64_t>() + offset, 1}});
        break;
      }
      ...
      case Opcode::AArch64_LDRDroW: {  // ldr dt, [xn, wm{, extend {amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[1].get<uint32_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[0].get<uint64_t>() + offset, 8}});
        break;
      }
      ...
      case Opcode::AArch64_LDRXpost: {  // ldr xt, [xn], #imm
        std::vector<simeng::MemoryAccessTarget> addresses;
        generateContiguousAddresses(srcRegVals[0].get<uint64_t>(), 1, dataSize_,
                                    addresses);
        setMemoryAddresses(addresses);
        break;
      }
      case Opcode::AArch64_LDRHHpost: {  // ldrh wt, [xn], #imm
        setMemoryAddresses({{srcRegVals[0].get<uint64_t>(), 2}});
        break;
      }
      ...
      case Opcode::AArch64_LDRHHroW: {  // ldrh wt, [xn, wm{, extend {#amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[1].get<uint64_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[0].get<uint64_t>() + offset, 2}});
        break;
      }
      ...
      case Opcode::AArch64_LDRQroX: {  // ldr qt, [xn, xm{, extend {#amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[1].get<uint64_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[0].get<uint64_t>() + offset, 16}});
        break;
      }
      case Opcode::AArch64_LDRSroW: {  // ldr st, [xn, wm{, extend {#amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[1].get<uint64_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[0].get<uint64_t>() + offset, 4}});
        break;
      }
      ...
      case Opcode::AArch64_LDR_PXI: {  // ldr pt, [xn{, #imm, mul vl}]
        ...
        const uint64_t base = srcRegVals[0].get<uint64_t>();
        const int64_t offset =
      ...
      case Opcode::AArch64_LDPXpre: {  // ldp xt1, xt2, [xn, #imm!]
        std::vector<simeng::MemoryAccessTarget> addresses;
        generateContiguousAddresses(
            srcRegVals[0].get<uint64_t>() + metadata.operands[2].mem.disp, 2,
            dataSize_, addresses);
        setMemoryAddresses(addresses);
        break;
      }
      ...
      case Opcode::AArch64_LDPXpost: {  // ldp xt1, xt2, [xn], #imm
        std::vector<simeng::MemoryAccessTarget> addresses;
        generateContiguousAddresses(srcRegVals[0].get<uint64_t>(), 2, dataSize_,
                                    addresses);
        setMemoryAddresses(addresses);
        break;
      }
      ...
      case Opcode::AArch64_ST1B: {  // st1b {zt.b}, pg, [xn, xm]
        ...
        const uint64_t base = srcRegVals[2].get<uint64_t>();
        const uint64_t offset = srcRegVals[3].get<uint64_t>();
      ...
      case Opcode::AArch64_SST1B_D_REAL: {  // st1b {zd.d}, pg, [xn, zm.d]
        ...
        const uint64_t base = srcRegVals[2].get<uint64_t>();
        const uint64_t* offset = srcRegVals[3].getAsVector<uint64_t>();
      ...
      case Opcode::AArch64_ST2D_IMM: {  // st2d {zt1.d, zt2.d}, pg, [<xn|sp>{,
        ...
        const uint64_t base = srcRegVals[3].get<uint64_t>();
        const int64_t offset =
            static_cast<int64_t>(metadata.operands[3].mem.disp);
      ...
      case Opcode::AArch64_SST1W_D_IMM: {  // st1w {zt.d}, pg, [zn.d{, #imm}]
        ...
        const uint64_t* n = srcRegVals[2].getAsVector<uint64_t>();
        const int64_t offset =
            static_cast<int64_t>(metadata.operands[2].mem.disp);
      ...
      case Opcode::AArch64_SST1W_IMM: {  // st1w {zt.s}, pg, [zn.s{, #imm}]
        ...
        const uint32_t* n = srcRegVals[2].getAsVector<uint32_t>();
        const int64_t offset = static_cast<int64_t>(
            static_cast<int32_t>(metadata.operands[2].mem.disp));
      ...
      case Opcode::AArch64_GLD1D_REAL: {  // ld1d {zt.d}, pg/z, [xn, zm.d]
        ...
        const uint64_t base = srcRegVals[1].get<uint64_t>();
        const uint64_t* offset = srcRegVals[2].getAsVector<uint64_t>();
      ...
      case Opcode::AArch64_GLD1D_IMM_REAL: {  // ld1d {zd.d}, pg/z, [zn.d{,
        ...
        const uint64_t* n = srcRegVals[1].getAsVector<uint64_t>();
        const int64_t offset =
            static_cast<int64_t>(metadata.operands[2].mem.disp);
      ...
      case Opcode::AArch64_GLD1W_D_SCALED_REAL: {  // ld1w {zd.d}, pg/z,
        ...
        const uint64_t n = srcRegVals[1].get<uint64_t>();
        const uint64_t* m = srcRegVals[2].getAsVector<uint64_t>();
      ...
      case Opcode::AArch64_GLD1W_SXTW_REAL: {  // ld1w {zd.s}, pg/z,
        ...
        const uint64_t n = srcRegVals[1].get<uint64_t>();
        const uint32_t* m = srcRegVals[2].getAsVector<uint32_t>();
      ...
      case Opcode::AArch64_ST1Fourv2s_POST: {  // st1 {vt.2s, vt2.2s, vt3.2s,
                                               // vt4.2s}, [xn], <#imm|xm>
        const uint64_t base = srcRegVals[4].get<uint64_t>();
        std::vector<MemoryAccessTarget> addresses;
        addresses.reserve(4);
      ...
      case Opcode::AArch64_ST1i8: {  // st1 {vt.b}[index], [xn]
        setMemoryAddresses({{srcRegVals[1].get<uint64_t>(), 1}});
        break;
      }
      case Opcode::AArch64_ST1i16_POST:
        [[fallthrough]];
      case Opcode::AArch64_ST1i16: {  // st1 {vt.h}[index], [xn]
        setMemoryAddresses({{srcRegVals[1].get<uint64_t>(), 2}});
        break;
      }
      ...
      case Opcode::AArch64_STPXpre: {  // stp xt1, xt2, [xn, #imm]!
        std::vector<simeng::MemoryAccessTarget> addresses;
        generateContiguousAddresses(
            srcRegVals[2].get<uint64_t>() + metadata.operands[2].mem.disp, 2,
            dataSize_, addresses);
        setMemoryAddresses(addresses);
        break;
      }
      ...
      case Opcode::AArch64_STPXpost: {  // stp xt1, xt2, [xn], #imm
        std::vector<simeng::MemoryAccessTarget> addresses;
        generateContiguousAddresses(srcRegVals[2].get<uint64_t>(), 2, dataSize_,
                                    addresses);
        setMemoryAddresses(addresses);
        break;
      }
      ...
      case Opcode::AArch64_STRBBroW: {  // strb wd,
                                        //  [xn, wm{, extend {#amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[2].get<uint32_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[1].get<uint64_t>() + offset, 1}});
        break;
      }
      case Opcode::AArch64_STRBBroX: {  // strb wd,
                                        //  [xn, xm{, extend {#amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[2].get<uint64_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[1].get<uint64_t>() + offset, 1}});
        break;
      }
      ...
      case Opcode::AArch64_STRDroW: {  // str dt, [xn, wm{, extend {#amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[2].get<uint32_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[1].get<uint64_t>() + offset, 8}});
        break;
      }
      ...
      case Opcode::AArch64_STRXpre: {  // str xt, [xn, #imm]!
        std::vector<simeng::MemoryAccessTarget> addresses;
        generateContiguousAddresses(
            srcRegVals[1].get<uint64_t>() + metadata.operands[1].mem.disp, 1,
            dataSize_, addresses);
        setMemoryAddresses(addresses);
        break;
      }
      ...
      case Opcode::AArch64_STRXpost: {  // str xt, [xn], #imm
        std::vector<simeng::MemoryAccessTarget> addresses;
        generateContiguousAddresses(srcRegVals[1].get<uint64_t>(), 1, dataSize_,
                                    addresses);
        setMemoryAddresses(addresses);
        break;
      }
      ...
      case Opcode::AArch64_STRHHroW: {  // strh wd,
        ...
        uint64_t offset =
            extendOffset(srcRegVals[2].get<uint32_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[1].get<uint64_t>() + offset, 2}});
        break;
      }
      ...
      case Opcode::AArch64_STRQroX: {  // str qt, [xn, xm{, extend {#amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[2].get<uint64_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[1].get<uint64_t>() + offset, 16}});
        break;
      }
      case Opcode::AArch64_STRSroW: {  // str st, [xn, wm{, extend {#amount}}]
        uint64_t offset =
            extendOffset(srcRegVals[2].get<uint32_t>(), metadata.operands[1]);
        setMemoryAddresses({{srcRegVals[1].get<uint64_t>() + offset, 4}});
        break;
      }
