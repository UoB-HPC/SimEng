...
void Instruction::execute() {
  ...
  switch (metadata.opcode) {
    ...
    case Opcode::RISCV_SD: {  // SD rs1,rs2,imm
      memoryData[0] = sourceRegValues[0];
      break;
    }
    case Opcode::RISCV_SLL: {  // SLL rd,rs1,rs2
      const int64_t rs1 = sourceRegValues[0].get<int64_t>();
      const int64_t rs2 =
          sourceRegValues[1].get<int64_t>() & 63;  // Only use lowest 6 bits
    ...
    case Opcode::RISCV_SLLW: {  // SLLW rd,rs1,rs2
      const int32_t rs1 = sourceRegValues[0].get<int32_t>();
      const int32_t rs2 =
          sourceRegValues[1].get<int32_t>() & 63;  // Only use lowest 6 bits
      int64_t out = signExtendW(static_cast<int32_t>(rs1 << rs2));
      results[0] = RegisterValue(out, 8);
      break;
    }
    case Opcode::RISCV_SLLIW: {  // SLLIW rd,rs1,shamt
      const int32_t rs1 = sourceRegValues[0].get<uint32_t>();
    ...
    case Opcode::RISCV_SRL: {  // SRL rd,rs1,rs2
      const uint64_t rs1 = sourceRegValues[0].get<uint64_t>();
      const uint64_t rs2 =
          sourceRegValues[1].get<uint64_t>() & 63;  // Only use lowest 6 bits
    ...
    case Opcode::RISCV_SRLW: {  // SRLW rd,rs1,rs2
      const uint32_t rs1 = sourceRegValues[0].get<uint32_t>();
      const uint32_t rs2 =
          sourceRegValues[1].get<uint32_t>() & 63;  // Only use lowest 6 bits
    ...
    case Opcode::RISCV_ADDW: {  // ADDW rd,rs1,rs2
      const int32_t rs1 = sourceRegValues[0].get<int32_t>();
      const int32_t rs2 = sourceRegValues[1].get<int32_t>();
    ...
    case Opcode::RISCV_LUI: {                 // LUI rd,imm
      uint64_t out = signExtendW(imm << 12);  // Shift into upper 20 bits
      results[0] = RegisterValue(out, 8);
      break;
    }
    case Opcode::RISCV_AUIPC: {  // AUIPC rd,imm
      const int64_t pc = instructionAddress_;
      const int64_t uimm = signExtendW(imm << 12);  // Shift into upper 20 bits
    ...
    case Opcode::RISCV_JALR: {  // JALR rd,rs1,imm
      branchAddress_ = (sourceRegValues[0].get<uint64_t>() + imm) &
    ...
    case Opcode::RISCV_AMOSWAP_W_AQ_RL: {
      ...
      int32_t rs2 = sourceRegValues[0].get<int32_t>();
    ...
    case Opcode::RISCV_AMOSWAP_D_AQ_RL: {
      ...
      uint64_t rs2 = sourceRegValues[0].get<uint64_t>();
      results[0] = RegisterValue(rd, 8);
      memoryData[0] = rs2;
      break;
    }
    ...
    case Opcode::RISCV_AMOADD_W_AQ_RL: {
      ...
      results[0] = RegisterValue(rd, 8);
      memoryData[0] =
          static_cast<int32_t>(rd + sourceRegValues[0].get<int64_t>());
      break;
    }
    ...
    case Opcode::RISCV_AMOADD_D_AQ_RL: {
      ...
      results[0] = RegisterValue(rd, 8);
      memoryData[0] =
          static_cast<int64_t>(rd + sourceRegValues[0].get<int64_t>());
      break;
    }
    ...
    case Opcode::RISCV_AMOMIN_W_AQ_RL: {
      ...
      memoryData[0] = std::min(memoryData[0].get<int32_t>(),
                               sourceRegValues[0].get<int32_t>());
      break;
    }
    ...
    case Opcode::RISCV_AMOMIN_D_AQ_RL: {
      ...
      results[0] = RegisterValue(rd, 8);
      memoryData[0] =
          static_cast<int64_t>(std::min(rd, sourceRegValues[0].get<int64_t>()));
      break;
    }
    ...
    case Opcode::RISCV_AMOMINU_W_AQ_RL: {
      ...
      memoryData[0] = std::min(memoryData[0].get<uint32_t>(),
                               sourceRegValues[0].get<uint32_t>());
      break;
    }
    ...
    case Opcode::RISCV_AMOMINU_D_AQ_RL: {
      uint64_t rd = memoryData[0].get<uint64_t>();
      results[0] = RegisterValue(rd, 8);
      memoryData[0] = static_cast<uint64_t>(
          std::min(rd, sourceRegValues[0].get<uint64_t>()));
      break;
    }
    ...
    case Opcode::RISCV_AMOMAX_W_AQ_RL: {
      results[0] = RegisterValue(signExtendW(memoryData[0].get<int32_t>()), 8);
      memoryData[0] = std::max(memoryData[0].get<int32_t>(),
                               sourceRegValues[0].get<int32_t>());
      break;
    }
    ...
    case Opcode::RISCV_AMOMAX_D_AQ_RL: {
      ...
      results[0] = RegisterValue(rd, 8);
      memoryData[0] =
          static_cast<int64_t>(std::max(rd, sourceRegValues[0].get<int64_t>()));
      break;
    }
    ...
    case Opcode::RISCV_AMOMAXU_D_AQ_RL: {
      ...
      results[0] = RegisterValue(rd, 8);
      memoryData[0] = static_cast<uint64_t>(
          std::max(rd, sourceRegValues[0].get<uint64_t>()));
      break;
    }
    ...
    case Opcode::RISCV_MULW: {  // MULW rd,rs1,rs2
      const uint32_t rs1 = sourceRegValues[0].get<uint32_t>();
      const uint32_t rs2 = sourceRegValues[1].get<uint32_t>();
    ...
    case Opcode::RISCV_FADD_D: {  // FADD.D rd,rs1,rs2
      setStaticRoundingModeThen([&] {
        const double rs1 = sourceRegValues[0].get<double>();
        const double rs2 = sourceRegValues[1].get<double>();
    ...
    case Opcode::RISCV_FADD_S: {  // FADD.S rd,rs1,rs2
      ...
      setStaticRoundingModeThen([&] {
        const float rs1 = checkNanBox(sourceRegValues[0]);
        const float rs2 = checkNanBox(sourceRegValues[1]);
    ...
    case Opcode::RISCV_FMADD_D: {  // FMADD.D rd,rs1,rs2,rs3
      ...
      setStaticRoundingModeThen([&] {
        const double rs1 = sourceRegValues[0].get<double>();
        const double rs2 = sourceRegValues[1].get<double>();
        const double rs3 = sourceRegValues[2].get<double>();
    ...
    case Opcode::RISCV_FMADD_S: {  // FMADD.S rd,rs1,rs2,rs3
      ...
      setStaticRoundingModeThen([&] {
        const float rs1 = checkNanBox(sourceRegValues[0]);
        const float rs2 = checkNanBox(sourceRegValues[1]);
        const float rs3 = checkNanBox(sourceRegValues[2]);
    ...
    case Opcode::RISCV_FMV_W_X: {  // FMV.W.X rd,rs1
      const float rs1 = sourceRegValues[0].get<float>();
