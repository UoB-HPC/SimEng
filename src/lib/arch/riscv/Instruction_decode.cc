...
void Instruction::invalidateIfNotImplemented() {
  ...
  if (metadata.opcode >= Opcode::RISCV_FADD_D &&
      metadata.opcode <= Opcode::RISCV_FADD_S)
    return;
  ...
  if (metadata.opcode >= Opcode::RISCV_FEQ_D &&
  ...
  //  if (metadata.opcode == Opcode::RISCV_FNMADD_D) return;
  //  if (metadata.opcode == Opcode::RISCV_FNMADD_S) return;
  ...
  //  if (metadata.opcode == Opcode::RISCV_FCVT_S_L) return;
  ...
  //  if (metadata.opcode == Opcode::RISCV_FCVT_L_D) return;
  //  if (metadata.opcode == Opcode::RISCV_FCVT_L_S) return;
  //
  //  if (metadata.opcode == Opcode::RISCV_FCVT_WU_D) return;
  //  if (metadata.opcode == Opcode::RISCV_FCVT_WU_S) return;
  //  if (metadata.opcode == Opcode::RISCV_FCVT_LU_D) return;
  //  if (metadata.opcode == Opcode::RISCV_FCVT_LU_S) return;
  //
  //  if (metadata.opcode == Opcode::RISCV_FCVT_D_WU) return;
  //  if (metadata.opcode == Opcode::RISCV_FCVT_S_WU) return;
  //  if (metadata.opcode == Opcode::RISCV_FCVT_D_LU) return;
  //  if (metadata.opcode == Opcode::RISCV_FCVT_S_LU) return;
  //
