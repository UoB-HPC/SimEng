...
bool MemoryMappedSystemRegisterBlock::put(uint16_t offset,
...
bool MemoryMappedSystemRegisterBlock::get(uint16_t offset,
                                          RegisterValue& value) {
  auto it = memoryMappedSystemRegisters.upper_bound(offset);
  if (it != memoryMappedSystemRegisters.begin()) {
    it--;
...
bool SystemRegisterMemoryInterface::putMemoryMappedSystemRegister(
    uint64_t address, const RegisterValue& value) {
...
bool SystemRegisterMemoryInterface::getMemoryMappedSystemRegister(
    uint64_t address, RegisterValue& value) {
  auto it = memoryMappedSystemRegisterBlocks_.upper_bound(address);
  if (it != memoryMappedSystemRegisterBlocks_.begin()) {
    it--;
    if (address - it->first < it->second->size()) {
...
int16_t Clint::updateSystemTimerRegisters(RegisterFileSet* regFile,
  ...
  if (ticked) {
    ...
    uint16_t mstatus_tag = static_cast<uint16_t>(
        architecture_.getSystemRegisterTag(SYSREG_MSTATUS));
    auto mstatus_bits =
        regFile->get({RegisterType::SYSTEM, mstatus_tag}).get<uint64_t>();
