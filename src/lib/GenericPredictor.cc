...
GenericPredictor::GenericPredictor(ryml::ConstNodeRef config)
    : btbBits_(config::SimInfo::getValue<uint8_t>(
          config["Branch-Predictor"]["BTB-Tag-Bits"])),
      satCntBits_(config::SimInfo::getValue<uint8_t>(
          config["Branch-Predictor"]["Saturating-Count-Bits"])),
      globalHistoryLength_(config::SimInfo::getValue<uint16_t>(
          config["Branch-Predictor"]["Global-History-Length"])),
      rasSize_(config::SimInfo::getValue<uint16_t>(
