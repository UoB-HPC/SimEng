...
ExecuteUnit::ExecuteUnit(
    ...
    std::function<int16_t(void)> isInterruptPending, BranchPredictor& predictor,
...
void ExecuteUnit::execute(std::shared_ptr<Instruction>& uop) {
  ...
  if (pendingInterruptId >= 0) {
    // std::cout << std::hex << "Execution encountered pending interrupt, PC 0x"
    // << uop->getInstructionAddress() << std::endl;
  ...
  if (uop->isBranch()) {
    ...
    if (uop->wasBranchMispredicted()) {
      // std::cout << std::dec << tickCounter_ << std::hex << ": Misprediction
      // iaddr: 0x" << uop->getInstructionAddress() << ", " <<
      // uop->getBranchPrediction().taken << std::endl;
