...
void DecodeUnit::tick() {
  ...
  for (size_t slot = 0; slot < output_.getWidth(); slot++) {
    ...
    // auto [misprediction, correctAddress] =
    // uop->checkEarlyBranchMisprediction(); if (misprediction) {
    //   earlyFlushes_++;
    //   shouldFlush_ = true;
    //   pc_ = correctAddress;

    //   if (!uop->isBranch()) {
    //     // Non-branch incorrectly predicted as a branch; let the predictor
    //     know predictor_.update(uop->getInstructionAddress(), false, pc_,
    //                       uop->getBranchType());
    //   }
...
void DecodeUnit::purgeFlushed() {
  ...
  if (microOps_.size()) microOps_.front()->setFlushed();
  microOps_.clear();
  input_.stall(false);
}
