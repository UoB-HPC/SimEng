...
void Process::loadElf(Elf& elf) {
  ...
  for (auto phdr : elf_phdrs) {
    ...
    min_addr = std::min(min_addr, downAlign(phdr.p_vaddr, PAGE_SIZE));
  ...
  for (auto phdr : elf_phdrs) {
    uint64_t startAddr = downAlign(phdr.p_vaddr, PAGE_SIZE);
    uint64_t endAddrMemSz = upAlign(phdr.p_vaddr + phdr.p_memsz, PAGE_SIZE);
...
void Process::loadInterpreter(Elf& elf) {
  ...
  for (auto phdr : interpreter->loadable_phdrs) {
    min_addr = std::min(min_addr, downAlign(phdr.p_vaddr, PAGE_SIZE));
    max_addr =
        std::max(max_addr, upAlign(phdr.p_vaddr + phdr.p_memsz, PAGE_SIZE));
  }
  ...
  for (auto& phdr : interpreter->loadable_phdrs) {
    uint64_t startAddr = map_addr + downAlign(phdr.p_vaddr, PAGE_SIZE);
    uint64_t endAddrMemSz =
        map_addr + upAlign(phdr.p_vaddr + phdr.p_memsz, PAGE_SIZE);
...
uint64_t Process::setupMemRegion<arch::aarch64::Architecture>(uint64_t brk) {
  ...
  uint64_t stack_guard_gap = (256 << 16);
...
void Process::archSetup<arch::aarch64::Architecture>() {
  ...
  uint64_t Implementer =
      std::stoi(ImplementerStr.substr(2, 1), nullptr, 16) * 16 +
      std::stoi(ImplementerStr.substr(3, 1), nullptr, 16);
  std::string VariantStr;
  config["CPU-Info"]["CPU-Variant"] >> VariantStr;
  uint64_t Variant = std::stoi(VariantStr.substr(2, 1), nullptr, 16);
  uint64_t Architecture = 0xf;
  std::string PartnumStr;
  config["CPU-Info"]["CPU-Part"] >> PartnumStr;
  uint64_t Partnum = std::stoi(PartnumStr.substr(2, 1), nullptr, 16) * 256 +
                     std::stoi(PartnumStr.substr(3, 1), nullptr, 16) * 16 +
                     std::stoi(PartnumStr.substr(4, 1), nullptr, 16);
