  ...
  if (likely(VMAlist_.size() > 0)) {
    ...
    if (!startAddr) {
      ...
      if (effStartAddr >= itr->vmEnd_ && space >= size) {
        ...
        // std::cout << "NEW VMA " << std::hex << vma.vmStart_ << std::dec
        //           << " to " << std::hex << vma.vmEnd_ << std::dec <<
        //           std::endl;
    ...
    */
    ...
    vma.vmStart_ = startAddr;
    vma.vmEnd_ = startAddr + size;
    // std::cout << "NEW VMA " << std::hex << vma.vmStart_ << std::dec << " to "
    //           << std::hex << vma.vmEnd_ << std::dec << std::endl;
...
int64_t MemRegion::unmapRegion(uint64_t addr, uint64_t length) {
  if (downAlign(addr, PAGE_SIZE) != addr) {
...
void MemRegion::printVmaList() {
  ...
  while (itr != VMAlist_.end()) {
    ...
    std::cout << "Has file: " << itr->hasFile() << std::endl;
    std::cout << std::endl;
    itr++;
    count++;
  }
