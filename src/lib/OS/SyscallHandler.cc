...
void SyscallHandler::handleSyscall() {
  ...
  switch (currentInfo_.syscallId) {
    case 17: {  // getcwd
      uint64_t bufPtr = currentInfo_.registerArguments[0].get<int64_t>();
      size_t size = currentInfo_.registerArguments[1].get<int64_t>();
      return readBufferThen(bufPtr, size, [=]() {
        char* path = getcwd(dataBuffer_.data(), size);
        ProcessStateChange stateChange = {
            ChangeType::REPLACEMENT, {currentInfo_.ret}, {bufPtr}};
        // Check for failure
        if (path == NULL) {
          return concludeSyscall(stateChange);
        }

        int64_t bytesRemaining = std::string(path).length();
    ...
    case 67: {  // pread64
      int64_t fd = currentInfo_.registerArguments[0].get<int64_t>();
      uint64_t bufPtr = currentInfo_.registerArguments[1].get<int64_t>();
      size_t count = currentInfo_.registerArguments[2].get<int64_t>();
      off_t offset = currentInfo_.registerArguments[3].get<int64_t>();
      return readBufferThen(bufPtr, count, [=]() {
        int64_t totalRead = pread64(fd, dataBuffer_.data(), count, offset);
        ProcessStateChange stateChange = {
            ChangeType::REPLACEMENT, {currentInfo_.ret}, {totalRead}};
        // Check for failure
        if (totalRead < 0) {
    ...
    case 79: {  // newfstatat AKA fstatat
      ...
      char* filename = (char*)calloc(PATH_MAX_LEN, sizeof(char));
      // for (int i = 0; i < PATH_MAX_LEN; i++)
      //   std::cerr << std::hex << unsigned(filename[i]) << std::dec << ",";
      // std::cerr << "\b" << std::endl;
      return readStringThen(
          filename, filenamePtr, PATH_MAX_LEN, [=](auto length) {
            // Invoke the kernel
...
void SyscallHandler::readStringThen(char* buffer, uint64_t address,
  ...
  // std::cerr << "readStringThen on address " << std::hex << address <<
  // std::dec
  //           << std::endl;
  if (maxLength <= 0) {
    return then(0);
  ...
  if (faultCode == simeng::OS::masks::faults::pagetable::DATA_ABORT ||
    ...
    // std::cerr << "\tABORTED OR IGNORED" << std::endl;
    return concludeSyscall({}, true);
  } else {
    ...
    // std::cerr << "\tAt max length" << std::endl;
...
uint64_t SyscallHandler::getDirFd(int64_t dfd, std::string pathname) {
  ...
  //   std::cout << std::hex << unsigned(pathname.c_str()[i]) << std::dec <<
  //   ",";
  // std::cout << "\b" << std::endl;
  // Resolve absolute path to target file
  char absolutePath[PATH_MAX_LEN];
  char* realpathRet = realpath(pathname.c_str(), absolutePath);
  ...
  if (dfd != -100) {
    ...
    if ((pathname.c_str()[0] == '\0') ||
        (strncmp(pathname.c_str(), absolutePath, strlen(absolutePath)) != 0)) {
      auto entry =
      ...
      // std::cout << "getDirFd:'" << pathname << "':" << dfd << ":" << dfd_temp
...
int64_t SyscallHandler::newfstatat(int64_t dfd, const std::string& filename,
  ...
  int64_t dirfd = SyscallHandler::getDirFd(dfd, filename);
  if (dirfd == -1) return EBADF;
  // std::cerr << "newfstatat flags 0x" << std::hex << flag << std::dec
  //           << std::endl;
...
char* SyscallHandler::getcwd(void* buf, size_t size) {
...
int64_t SyscallHandler::openat(int64_t dfd, const std::string& filename,
  ...
  if (dirfd == -1) {
    // std::cerr
    //     << "[SimEng:SyscallHandler] Error getting dirfd for file at pathname:
    //     "
    //     << new_pathname << " with errno " << EBADF << std::endl;
    return EBADF;
  }
...
int64_t SyscallHandler::pread64(int64_t fd, void* buf, size_t count,
                                off_t offset) {
  auto entry = OS_->getProcess(currentInfo_.threadId)->fdArray_->getFDEntry(fd);
  if (!entry.isValid()) {
    return EBADF;
