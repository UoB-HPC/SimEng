...
void ModelConfig::setExpectations(bool isDefault) {
  ...
  expectations_["Process-Image"].addChild(
      expectations_.create<uint64_t>("Mmap-Size", 200000));
    ...
    for (const auto& field : subFields) {
      ...
      if (result == "Success") {
        ...
        std::cerr
            ...
            << regionSize << "\" passed in config file due to \"" << result
            << "\" error. Cannot continue with config validation. Exiting."
            << std::endl;
        exit(1);
      }
  ...
  // Memory-Hierarchy
  expectations_.addChild(expectations_.create("Memory-Hierarchy"));

  expectations_["Memory-Hierarchy"].addChild(
      expectations_.create<uint64_t>("Cache-Line-Width", 64));
  ...
  expectations_["Memory-Hierarchy"].addChild(expectations_.create("L1-Data"));

  expectations_["Memory-Hierarchy"]["L1-Data"].addChild(
      expectations_.create<uint64_t>("Access-Latency", 1));
  ...
  expectations_.addChild(
      expectations_.create<std::string, true>("Environment-Variables", ""));
