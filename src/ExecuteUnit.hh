...
/** An execute unit for an in-order pipeline. Executes instructions and forwards results to the decode stage. */
class ExecuteUnit {
 public:
  /** Constructs an execute unit with references to an input and output buffer, the decode unit, the currently used branch predictor, and a pointer to process memory. */
  ...
  /** Tick the execute unit. Executes the current instruction and forwards the results back to the decode stage. */
  void tick();
