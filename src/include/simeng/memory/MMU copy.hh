...
struct reqEntry {
  ...
  reqEntry(uint64_t reqId, std::shared_ptr<Instruction> insn,
           std::shared_ptr<MemoryAccessTarget> target,
           uint16_t totalPacketsRemaining, bool failed, bool setMSHRS,
           uint64_t cycleSent)
      : reqId_(reqId),
        insn_(insn),
        target_(target),
        ...
        failed_(failed),
        setMSHRS_(setMSHRS),
        cycleSent_(cycleSent) {}

  uint64_t reqId_ = 0;

  std::shared_ptr<Instruction> insn_ = nullptr;

  std::shared_ptr<MemoryAccessTarget> target_ = nullptr;
  ...
  bool setMSHRS_ = false;

  uint64_t cycleSent_ = 0;
};
...
struct tlbMSHR {
  ...
  std::deque<std::shared_ptr<Instruction>> missQueue_ = {};
};
...
class MMU {
 public:
  ...
  requestSuccess requestPrefetch(const std::shared_ptr<Instruction>& uop);
  ...
  void markMiss(uint64_t vAddr);

  void requestPrefetch(uint64_t vAddr);
 ...
 private:
   ...
   * Each inner vector represents a single instruction. */
  std::array<
      std::vector<std::vector<std::pair<std::unique_ptr<MemPacket>, uint64_t>>>,
      2>
      loadsStores_;
  ...
  uint64_t l2TLBmissPen_ = 1;
  uint64_t l1TLBMSHRSize_ = 8;
