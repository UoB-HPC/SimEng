...
template <typename T>
uint8_t ccmn_imm(srcValContainer& operands,
 ...
 * Returns single value of type uint8_t. */
template <typename T>
uint8_t ccmp_imm(srcValContainer& operands,
                 const simeng::arch::aarch64::InstructionMetadata& metadata) {
  if (conditionHolds(metadata.cc, operands[0].get<uint8_t>())) {
    uint8_t nzcv;
    std::tie(std::ignore, nzcv) = addWithCarry(
 ...
 * Returns single value of type uint8_t. */
template <typename T>
uint8_t ccmp_reg(srcValContainer& operands,
                 const simeng::arch::aarch64::InstructionMetadata& metadata) {
  if (conditionHolds(metadata.cc, operands[0].get<uint8_t>())) {
    uint8_t nzcv;
...
template <typename T>
T cs_4ops(srcValContainer& operands,
          const simeng::arch::aarch64::InstructionMetadata& metadata,
