...
RegisterValue vecBitwiseInsert(srcValContainer& sourceValues, bool isBif) {
  const auto d = sourceValues[0].getAsVector<uint64_t>();
  const auto n = sourceValues[1].getAsVector<uint64_t>();
  const auto m = sourceValues[2].getAsVector<uint64_t>();
  uint64_t out[2] = {0};
  for (int i = 0; i < (I / 8); i++) {
    out[i] =
...
RegisterValue vecCompare(srcValContainer& sourceValues, bool cmpToZero,
  ...
  const auto n = sourceValues[0].getAsVector<T>();
  const auto m = !cmpToZero ? sourceValues[1].getAsVector<T>() : safePointer<T>();
  T out[16 / sizeof(T)] = {0};
  for (int i = 0; i < I; i++) {
...
RegisterValue vecCountPerByte(srcValContainer& sourceValues) {
  const auto n = sourceValues[0].getAsVector<uint8_t>();
  T out[16 / sizeof(T)] = {0};
  for (int i = 0; i < I; i++) {
...
RegisterValue vecFcvtl(srcValContainer& sourceValues, bool isFcvtl2) {
  const auto n = sourceValues[0].getAsVector<N>();
  D out[16 / sizeof(D)] = {0};
...
RegisterValue vecUMaxP(srcValContainer& sourceValues) {
  ...
  T temp[2 * I];
  n.copyTo(temp, sizeof(T) * I);
  m.copyTo(temp + (sizeof(T) * I),sizeof(T) * I);
...
RegisterValue vecShrnShift_imm(
    ...
    const simeng::arch::aarch64::InstructionMetadata& metadata,
    bool shrn2 = false) {
  const auto n = sourceValues[0].getAsVector<Ta>();
...
RegisterValue vecXtn(srcValContainer& sourceValues, bool isXtn2) {
  const auto d = isXtn2 ? sourceValues[0].getAsVector<D>() : safePointer<D>();
  const auto n = isXtn2 ? sourceValues[1].getAsVector<N>() : sourceValues[0].getAsVector<N>();
...
RegisterValue vecTbl(
  ...
  for (uint8_t i = 0; i < n_table_regs; i++) {
    const auto currentVector = sourceValues[i].getAsVector<uint8_t>();
...
RegisterValue vecRev(srcValContainer& sourceValues) {
  const auto source = sourceValues[0].getAsVector<T>();
