...
#include "Prefetchers/perceptronFilteringPrefetch.hh"
...
PerceptronFilteringPrefetch::PerceptronFilteringPrefetch(ComponentId_t id, Params& params) : CacheListener(id, params) {
...
PerceptronFilteringPrefetch::~PerceptronFilteringPrefetch() {}

void PerceptronFilteringPrefetch::notifyAccess(const CacheListenerNotification& notify) {
...
void PerceptronFilteringPrefetch::registerResponseCallback(Event::HandlerBase *handler) {
    registeredCallbacks.push_back(handler);
}

void PerceptronFilteringPrefetch::printStats(Output& out) {
