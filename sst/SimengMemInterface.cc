    ...
    AggregateWriteRequest* aggrReq, uint64_t addrStart, uint64_t size)
{
    ...
    // Determine the number of cache-lines needed to store the data in the write request
    int cacheLinesNeeded = getCacheLinesNeeded(size);
    /*
        This check here increments the data index to a value indexing the portion of data
        which succeeds the portion data already copied incase the request spans multiple cache-lines.
        In reference to the diagram above, this check will succeed only for cache-line 2.
    */
    ...
    AggregateReadRequest* aggrReq, uint64_t addrStart, uint64_t size)
{
    std::vector<StandardMem::Request*> requests;
    std::vector<uint64_t> req_ids;
    // Get the number of cache-lines needed to read the data requested by the read request.
    int cacheLinesNeeded = getCacheLinesNeeded(size);
    ...
    for (uint64_t id: req_ids) {
        /* 
            Insert a key-value pair of SST request id and AggregatedReadRequest reference
            in the aggregation map. These key-value pairs will later be used to store
            read response data recieved from SST. This models a many-to-one relation between 
            multiple SST requests and a SimEng read request.
        */
...
void SimengMemInterface::aggregatedReadResponses(AggregateReadRequest* aggrReq) {
    ...
    // Loop through the ordered map and merge the data in order inside the mergedData vector.
    // Also remove entries from the aggregation_map as we loop through each SST Request id.
    ...
    // Send the completed read request back to SimEng via the completed_read_requests
    // queue.
...
void SimengMemInterface::SimengMemHandlers::handle(StandardMem::ReadResp* rsp) {
    ...
    /* 
        After succesful retrieval of AggregatedReadRequest from aggregation_map the
        response data is stored inside the AggregatedReadRequest in an ordered map.
        It is neccesary to maintain order in which the orginal read request from SimEng
        was split into otherwise garbage values will be obtained upon merging.
        An ordered map is used here because SST::StandardMem::Request ids are generated
        using an atomic incrementing couter.
        Reference - "interfaces/stdMem.(hh/cc)" (SST-Core)
    */
