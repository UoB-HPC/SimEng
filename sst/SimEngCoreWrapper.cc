...
void SimEngCoreWrapper::processMemoryImage() {
  ...
  for (const auto& header : headers32_) {
    if (header.type == 1) {  // LOAD
      ...
      dataMemory_->sendProcessImageToSST(imagePointer, header.memorySize,
                                         header.virtualAddress);
    }
  }
  std::cout
      << "[SSTSimEng:SimEngCoreWrapper] Done exporting elf data into SST memory"
      << std::endl;
...
void SimEngCoreWrapper::finish() {
  ...
  std::cout << "\n[SimEng] Finished " << std::dec << iterations_ << " ticks in "
            << duration << "ms (" << std::round(khz) << " kHz, "
