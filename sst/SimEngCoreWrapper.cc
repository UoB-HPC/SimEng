...
// SimEngCoreWrapper::SimEngCoreWrapper(SST::ComponentId_t id, SST::Params&
// params)
//     : SST::Component(id) {
//   output_.init("[SSTSimEng:SimEngCoreWrapper] " + getName() + ":@p:@l ", 999,
//   0,
//                SST::Output::STDOUT);
...
//   // dataMemory_ = std::make_shared<SimEngMemInterface>(sstMem_,
//   // cacheLineWidth_,
//   //                                                    maxAddrMemory_,
//   debug_);

//   // handlers_ = new SimEngMemInterface::SimEngMemHandlers(*dataMemory_,
//   // &output_);
...
//       output_.verbose(
//           CALL_INFO, 1, 0,
//           "Received cntx msg from %s\n\t-TID: %llu\n\t-PC: "
//           "%llu\n\t-SP: %llu\n\t-progByteLen_: %llu\n\t-Reg 0|0: "
//           "%d\n\t-Reg 0|1: %d\n\t-Reg 1|0: %llu\n\t-Reg 1|1: %llu\n",
//           cntxEv->getSource().c_str(), cntx.TID, cntx.pc, cntx.sp,
//           cntx.progByteLen, cntx.regFile[0][0].get<uint32_t>(),
//           cntx.regFile[0][1].get<uint32_t>(),
//           cntx.regFile[1][0].get<uint64_t>(),
//           cntx.regFile[1][1].get<uint64_t>());
//       canEnd_ = true;
//       break;
//     }
//     default:
...
//   // std::cout << "\n[SimEng] Finished " << iterations_ << " ticks in " <<
//   // duration
//   //           << "ms (" << std::round(khz) << " kHz, " <<
//   std::setprecision(2)
//   //           << mips << " MIPS)" << std::endl;
// }
...
//   //         "Please increase the memory allocated to memHierarchy.memBackend
//   //         and " "ensure it is consistent with \'max_addr_memory\' and "
//   //         "\'addr_range_end\'. \n");
//   //     primaryComponentOKToEndSim();
//   //     std::exit(EXIT_FAILURE);
//   //   }
//   // // If testing is enabled populate heap if heap values have been
//   specified.
...
//       and
//       // we encounter another single quote, capture the substring enclosed by
//       a
...
//       and
//       // we encounter another double quote, capture the substring enclosed by
//       a
