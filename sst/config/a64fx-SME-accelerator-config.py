...
SIMENG_SRC_DIR = "/home/milosz/GitRepos/SimEng/"
A64FX_SIMENG_MODEL_CONFIG_PATH = SIMENG_SRC_DIR + "configs/sst-cores/a64fx-sst_accelerated.yaml"
BINARY_PATH = "/home/milosz/GitRepos/sme_test"
cpu = sst.Component("core", "sstsimeng.simengcore")
cpu.addParams({
    "simeng_config_path": A64FX_SIMENG_MODEL_CONFIG_PATH,
    "executable_path": BINARY_PATH,
    "executable_args": "1 10 10 10",
    "clock": A64FX_CLOCK,
    "max_addr_memory": memprops["end_addr"],
    "cache_line_width": A64FX_CLW,
    "external_accelerators": ["AArch64_SME"],
    "debug": DEBUG_CORE > 0,
...
# ----------------------------------------- SSTSimEng Accelerator ------------------------------------

# Using sst-info sstsimeng.simengaccelerator to get all cache parameters, ports and subcomponent slots.
A64FX_SIMENG_MODEL_CONFIG_PATH = SIMENG_SRC_DIR + "configs/sst-cores/a64fx-sst_SME-accelerator.yaml"
accelerator = sst.Component("accelerator", "sstsimeng.simengaccelerator")
accelerator.addParams({
    "simeng_config_path": A64FX_SIMENG_MODEL_CONFIG_PATH,
    "clock": A64FX_CLOCK,
    "max_addr_memory": memprops["end_addr"],
    "cache_line_width": A64FX_CLW,
    "debug": DEBUG_ACCELERATOR > 0,
    "debug_level": DEBUG_ACCELERATOR,
})

# Instantiating the StandardInterface which communicates with the SST memory model.
accelerator_interface = accelerator.setSubComponent("memory", "memHierarchy.standardInterface")

# ----------------------------------------- SSTSimEng Accelerator ------------------------------------


# -------------------------------------- CPU/Accelerator Memory Bus ----------------------------------
...
link_cpu_accelerators = sst.Link("link_cpu_accelerator_link")
link_cpu_accelerators.connect((cpu, "accelerator_link", "0ps"), (accelerator, "core_link", "0ps"))

link_cpu_bus = sst.Link("link_cpu_bus_link")
link_cpu_bus.connect((interface, "port", "0ps"), (bus, "high_network_0", "0ps"))
link_accelerator_bus = sst.Link("link_accelerator_bus_link")
link_accelerator_bus.connect((accelerator_interface, "port", "0ps"), (bus, "high_network_1", "0ps"))

link_bus_l1cache = sst.Link("link_bus_l1cache_link")
link_bus_l1cache.connect((bus, "low_network_0", "0ps"), (l1cache, "high_network_0", "0ps"))
