...
The logic held in ``src/lib/arch/aarch64/Instruction_decode.cc`` is primarily associated with converting the provided Capstone instruction metadata into the appropriate SimEng ``instruction`` format. Additionally, an instruction's group/identifiers are defined here through operand usage and opcode values. For the AArch64 architecture model, the following groups are defined:

- Arithmetic
- Shift (An instruction containing a shift operator as opposed to a shift instruction (e.g. LSL))
- Multiply
- Divide
- ASIMD (Advanced SIMD)
- Load
- Store
- Branch
- Predicate (An SVE instruction writing to a predicate register)

For the instruction group value (as described :ref:`here <instruction-group>`), the Arithmetic group is represented by the lowest bit and Predicate group the highest. Beyond this, identifiers for SVE, return, and branch and link instructions exist through getter-style functions.
