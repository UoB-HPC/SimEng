...
Process::Process(const std::vector<std::string>& commandLine,
  ...
  // Align heap start to a pageSize boundary
  uint64_t heapStart = roundUpMemAddr(elf.getMaxVirtAddr(), 4096);
  ...
  memRegion_ = MemRegion(stackSize, heapSize, size, stackPtr, heapStart,
                         pageSize_, mmapStart);
  /*
  for (auto header : elf.getProcessedHeaders()) {
    memRegion_.addInitalVMA(header->headerData, header->virtualAddress,
                            header->memorySize, PTLoad);
  }*/
...
// void Process::addInitialVMA(VMA* vma) {}
